...
struct P16InfrastructureTests {
  ...
  func apiTransportDecodesBackendTimestamps() async throws {
    ...
    let fractionalOffset = fractional.timestamp.timeIntervalSince(offset.timestamp)
    #expect(fractionalOffset > 0.099 && fractionalOffset < 0.101)
  }
