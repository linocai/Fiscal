...
/// Non-JSON response boundary. Header names stay case-insensitive at the
/// consumer boundary so platform HTTP implementations cannot weaken checks.
public struct APIArtifactResponse: Sendable {
...
public actor APITransport {
    ...
    public func rawDataGET(
        ...
        try await rawDataGETResponse(path, query: query, accept: accept).data
    }

    /// Authenticated artifact transfer with the selected response metadata.
    /// Callers that make a revision claim must bind it to this response, rather
    /// than attributing bytes to the revision they happened to request.
    public func rawDataGETResponse(
        _ path: String,
        query: [URLQueryItem] = [],
        accept: String = "application/octet-stream"
    ...
    /// Authenticated non-JSON artifact request for the small number of
    /// contracts which require a JSON body (the encrypted Archive export).
    /// It intentionally shares the same response/error handling as GET
    /// artifacts and never caches a sensitive transfer.
