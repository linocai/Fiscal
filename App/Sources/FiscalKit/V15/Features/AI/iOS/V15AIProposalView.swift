...
public struct V15AIProposalView: View {
    ...
    private var header: some View {
        VStack(alignment: .leading, spacing: V15Spacing.sm) {
            ...
            Text("AI 只整理提案。字段必须由你保存确认，之后才会出现可用的人工执行动作。").font(V15Typography.body).foregroundStyle(V15Palette.ink.color.opacity(0.68)).fixedSize(horizontal: false, vertical: true)
            HStack(spacing: V15Spacing.xs) {
                Image(systemName: "hand.raised.fill").foregroundStyle(V15Palette.teal.color)
                Text("服务端契约 · effective_auto_execute=false").font(V15Typography.secondary.weight(.semibold)).foregroundStyle(V15Palette.teal.color)
            }
    ...
    @ViewBuilder private var queueSurface: some View {
        V15Section("提案队列", detail: "\(model.pendingCount) 笔待确认") {
            switch model.phase {
            case .idle, .loading: V15LoadingSkeleton().accessibilityIdentifier("v15.f3f.queue.loading")
            case .empty: V15EmptyState(title: "没有提案", explanation: "输入一段记账文字，提案会停在这里等待你确认。").accessibilityIdentifier("v15.f3f.queue.empty")
    ...
    @ViewBuilder private var detailSurface: some View {
        V15Section("提案详情") {
            ...
            case .idle: V15EmptyState(title: "选择一项提案", explanation: "查看解析置信度、缺失字段、原因和质量事件。")
        ...
        NavigationStack {
            ScrollView {
                VStack(alignment: .leading, spacing: V15Spacing.lg) {
                    ...
                    Text("保存会把完整 draft 与当前 expected_version 发给服务端；保存成功后仍不会执行记账。").font(V15Typography.secondary).foregroundStyle(V15Palette.ink.color.opacity(0.68)).fixedSize(horizontal: false, vertical: true)
                    ...
                    if let proposal = model.selectedProposal {
                        V15ActionButton("人工执行已确认内容", symbol: "checkmark.circle", kind: .primary, disabledReasons: model.actionReasons(.execute, proposal: proposal)) {
