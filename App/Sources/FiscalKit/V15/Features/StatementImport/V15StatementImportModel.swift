...
public final class V15StatementImportModel {
    ...
    } catch let failure as V15Failure { guard token == generation else { return }; fail(failure) } catch { guard token == generation else { return }; fail(.init(kind: .transport, message: "账单导入失败。")) } }
    ...
    public func readConfirmationReceipt() async { guard let batch, let key = confirmationKey, case .responseUnknown = phase else { return }; generation &+= 1; let token = generation; do { let value = try await services.statementImports.confirmationReceipt(importID: batch.id, idempotencyKey: key, readCachePolicy: .reloadIgnoringCache); guard token == generation else { return }; await reloadWorkbench(); guard token == generation else { return }; receipt = value; phase = .completed(value) } catch let failure as V15Failure { guard token == generation else { return }; if failure.kind != .cancelled { fail(failure) } } catch { guard token == generation else { return }; fail(.init(kind: .transport, message: "检查确认结果失败。")) } }
