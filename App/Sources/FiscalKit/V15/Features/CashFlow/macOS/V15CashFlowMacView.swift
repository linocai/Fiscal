...
public struct V15CashFlowMacView: View {
    ...
    public init(services: V15Services, offlineSnapshotAt: Date? = nil, initialGalleryScenario: String? = nil, initialItem: V15CashFlowItem? = nil) {
    ...
    @ViewBuilder private var surface: some View {
        ...
        case .empty: V15EmptyState(title: section == .active ? "没有未来现金流" : "本月没有历史", explanation: "这个范围内暂时没有事项。", actionTitle: "刷新") { Task { await model.refresh() } }.accessibilityIdentifier("v15.f3d.mac.empty")
    ...
    private var inspector: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: V15Spacing.md) {
                ...
                else { V15EmptyState(title: "选择一项现金流", explanation: "查看状态、金额、来源与可用操作。") }
                mutationSurface
            }
            .padding(V15Spacing.md).frame(maxWidth: .infinity, alignment: .leading)
        }
    ...
    private func itemInspector(_ item: V15CashFlowItem) -> some View {
        VStack(alignment: .leading, spacing: V15Spacing.md) {
            ...
            if item.isSystem {
                ...
                V15Section("可用操作") {
                    if let preview = model.confirmPreview, preview.itemBefore.id == item.id {
                        ...
                        V15ActionButton(model.confirmCommitIsInFlight ? "正在提交" : "确认本次", kind: .secondary, disabledReasons: model.actionReasons(.confirm, for: item)) { Task { await model.commitConfirmPreview() } }.accessibilityIdentifier("v15.f3d.mac.confirm.commit")
                    } else {
                        V15ActionButton("查看确认影响", kind: .secondary, disabledReasons: model.actionReasons(.confirm, for: item)) { Task { await model.previewConfirm(item) } }.accessibilityIdentifier("v15.f3d.mac.confirm.preview")
                    }
            ...
            if !item.creditCycleParts.isEmpty { V15Section("账期构成") { ForEach(item.creditCycleParts) { part in Text("\(part.periodStart)–\(part.periodEnd) · \(money(part.remainingMinor))").font(V15Typography.secondary) } } }
        }
    }
    ...
    @ViewBuilder private var recoveryActions: some View {
        if model.hasUnknownStableAttempt { V15ActionButton("安全检查保存结果", kind: .secondary, disabledReason: model.isOffline ? .init(code: "offline_read_only", message: "离线时不能检查保存结果。", fieldPath: nil) : nil) { Task { await model.retryUnknownStable() } }.accessibilityIdentifier("v15.f3d.mac.unknown.retry-same-key"); V15ActionButton("停止恢复", kind: .quiet) { model.abandonUnknownStable() } }
    ...
    private func visibleEditorIssues(_ path: String) -> [V15FieldIssue] {
        ...
        return fieldIssues(model.editorIssues, path)
    }
    private func displayReasons(_ values: [V15DisabledReason]) -> [V15DisabledReason] { values.filter { $0.fieldPath == nil } }
    ...
    private func sourceLabel(_ item: V15CashFlowItem) -> String { item.systemKind == .creditCycle ? "来自信用账单" : item.systemKind == .reimbursement ? "来自报销单" : item.source == "manual" ? "手工" : "其他" }
    ...
    private var editorTitle: String { switch model.editorMode { case .create: "新建现金流"; case .edit: "修改现金流"; case .settle: "兑现入账"; case .systemEdit: "自动事项显示"; case .none: "详情" } }
    private func editOpenReasons(_ item: V15CashFlowItem) -> [V15DisabledReason] { var values: [V15DisabledReason] = []; if model.writeLocked { values.append(.init(code: "write_locked", message: "上一项操作还没有完成。", fieldPath: nil)) }; if !item.allows(.edit) { values.append(.init(code: "server_action_unavailable", message: "当前状态不能修改。", fieldPath: "actions")) }; return values }
    private func settleOpenReasons(_ item: V15CashFlowItem) -> [V15DisabledReason] { var values: [V15DisabledReason] = []; if model.writeLocked { values.append(.init(code: "write_locked", message: "上一项操作还没有完成。", fieldPath: nil)) }; if !item.allows(.settle) { values.append(.init(code: "server_action_unavailable", message: "当前状态不能入账。", fieldPath: "actions")) }; return values }
    private func systemOpenReasons(_ item: V15CashFlowItem) -> [V15DisabledReason] { var values: [V15DisabledReason] = []; if model.writeLocked { values.append(.init(code: "write_locked", message: "上一项操作还没有完成。", fieldPath: nil)) }; if item.systemKind == .creditCycle { values.append(.init(code: "credit_projection_read_only", message: "信用账单安排只供查看。", fieldPath: "system_kind")) }; if !item.allows(.edit) { values.append(.init(code: "server_action_unavailable", message: "当前状态不能修改。", fieldPath: "actions")) }; return values }
