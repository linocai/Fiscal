...
public struct V15MasterDataView: View {
    ...
    private var merchantEditor: some View { Group { V15Section("商户") { V15Field("商户名称", text: $model.merchantName, prompt: "例如 咖啡店", issues: model.fieldIssues).accessibilityIdentifier("v15.f1c.merchant.name"); V15Field("别名（用、分隔）", text: $model.merchantAliases, prompt: "Coffee", issues: []) }; V15Section("交易映射") { V15Field("交易 ID", text: $model.mappingTransactionID, prompt: "UUID", issues: model.fieldIssues); V15InspectorAction("读取当前映射", action: { Task { await model.loadMapping() } }); V15InspectorAction("确认映射到当前商户", disabledReason: model.selectedMerchant == nil ? .init(code: "merchant_required", message: "请先选择一个商户。", fieldPath: nil) : nil, action: { Task { await model.confirmMapping() } }); if model.mapping != nil { V15InspectorAction("解除当前映射", action: { Task { await model.releaseMapping() } }) } }; Text("商户归档没有后端接口，因此这里不会提供归档动作。").font(V15Typography.secondary).foregroundStyle(V15Palette.ink.color.opacity(0.65)) } }
    private func archiveAction(title: String, action: @escaping () -> Void) -> some View { V15InspectorAction(title, detail: "以服务器版本为准，归档不会删除历史事实。", disabledReason: model.writeDisabledReason, action: action) }
    private var mergeContent: some View { NavigationStack { ScrollView { VStack(alignment: .leading, spacing: V15Spacing.md) { Text("合并预览").font(V15Typography.surfaceTitle); transformStatus; if let preview = model.transformPreview { Text("服务器将原子重归类 \(preview.source.transactionCount) 笔账目。") .font(V15Typography.secondary); ForEach(preview.childMappingRequirements, id: \.sourceChildID) { item in VStack(alignment: .leading) { Text("子分类：\(item.sourceChildName)").font(V15Typography.secondary); if let selected = model.childMappings[item.sourceChildID] { Picker("归位到", selection: Binding(get: { model.childMappings[item.sourceChildID] ?? selected }, set: { model.childMappings[item.sourceChildID] = $0 })) { ForEach(item.targetChildIDs, id: \.self) { id in Text(model.visibleCategories.first(where: { $0.id == id })?.name ?? "可用分类").tag(id) } }.pickerStyle(.menu) } else { Text("服务器未提供此子分类的可用归位目标。") .font(V15Typography.secondary).foregroundStyle(.red) } } }; let noTarget = preview.childMappingRequirements.contains { $0.targetChildIDs.isEmpty }; V15ActionButton("确认原子合并", symbol: "arrow.triangle.merge", disabledReasons: (model.writeDisabledReason.map { [$0] } ?? []) + (noTarget ? [.init(code: "merge_target_missing", message: "服务器没有提供所有子分类的归位目标。", fieldPath: nil)] : []), action: { Task { if await model.commitMerge() { mergeSheet = false; model.invalidatePreview() } } }).accessibilityIdentifier("v15.f1c.merge.commit") } else { V15ServiceErrorState(message: model.transformMessage ?? "无法读取合并预览。", retry: { Task { await model.retryTransformPreview() } }) } }.padding(V15Spacing.md) }.toolbar { ToolbarItem(placement: .cancellationAction) { Button("取消") { mergeSheet = false; model.invalidatePreview() } } } } }
