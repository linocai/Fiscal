...
public struct V15CreditView: View {
    ...
    public init(services: V15Services, offlineSnapshotAt: Date? = nil, offlineSnapshotProvider: (@MainActor @Sendable () -> Date?)? = nil, initialGalleryScenario: String? = nil, fixtureReconnectAction: (() -> Void)? = nil, initialCycle: V15CreditCycle? = nil, initialAccountID: UUID? = nil) {
        self.services = services; _model = State(initialValue: .init(services: services, offlineSnapshotAt: offlineSnapshotAt, offlineSnapshotProvider: offlineSnapshotProvider))
    ...
    private var header: some View {
        HStack(alignment: .top) {
            V22PageHeader("信用账期", symbol: "creditcard", subtitle: "欠款、还款日与账单，一目了然")
    ...
    @ViewBuilder private var content: some View {
        ...
        case .empty:
            V15EmptyState(title: "目前没有信用账户", explanation: "到账户页添加信用账户后，这里会显示账单。")
        case .loaded:
            if let account = model.selectedAccount { accountSummary(account) }
            if model.selectedAccount?.cycleMode != .onDemand {
    ...
    private func accountSummary(_ account: V15CreditAccountSummary) -> some View {
        V15Section("信用概览", detail: account.cycleMode == .onDemand ? "随借随还 · 不设额度或固定还款日" : "账单日 \(account.statementDay.map(String.init) ?? "—") · 还款日 \(account.dueDay.map(String.init) ?? "—")") {
            ...
            if account.activeInstallmentCount > 0 || account.futureScheduledGrossMinor > 0 {
                V15PreviewState {
                    VStack(alignment: .leading, spacing: V15Spacing.xxs) {
                        ...
                        Text("未来分期安排与账户欠款可能覆盖同一笔本金；全额结清预览会统一核对剩余金额。")
    ...
    private func creditMetric(_ title: String, _ value: V15MinorUnits?, _ direction: V15MoneyDirection) -> some View {
        V22Metric(title, minorUnits: value, direction: direction)
    }
    ...
    private var scheduleSheet: some View {
        NavigationStack {
            ScrollView {
                VStack(alignment: .leading, spacing: V15Spacing.md) {
                    V22PageHeader("调整账期", symbol: "calendar", subtitle: "先查看影响，再确认修改")
                    V22FlowProgress(["设置", "查看影响", "确认"], current: model.schedulePreview == nil ? 0 : 1)
                    V22FormSection("账单规则") {
                        V15PickerRow("账期方式", selection: $model.cycleMode) {
                    ...
                    reasons
                    V15ActionButton("查看修改影响", symbol: "arrow.right", disabledReason: model.schedulePreviewDisabledReason, showsDisabledReasons: false, accessibilityIdentifier: "v15.f3b1.schedule.preview") { Task { await model.requestSchedulePreview() } }
                        .disabled(!model.canRequestSchedulePreview)
                    if let reason = model.schedulePreviewDisabledReason {
                        Text(reason.message).font(V15Typography.secondary).foregroundStyle(V15Palette.danger.color).accessibilityIdentifier("v15.f3b1.schedule.preview-reason")
                    }
                    ...
                    if model.scheduleCommandDisabledReason != nil {
                        ...
                        if let reason = model.scheduleDisabledReason {
                            Text(reason.message).font(V15Typography.secondary).foregroundStyle(V15Palette.danger.color).accessibilityIdentifier("v15.f3b1.schedule.commit-reason")
                        }
                    }
                    previewSurface
                }.padding(V15Spacing.md)
    ...
    @ViewBuilder private var reasons: some View {
        if !model.scheduleIssues.isEmpty { V15Section("请先修正") { ForEach(model.scheduleIssues, id: \.code) { Text($0.message).font(V15Typography.secondary).foregroundStyle(V15Palette.danger.color) } }.accessibilityIdentifier("v15.f3b1.schedule.local-reasons") }
        if !model.scheduleServerFieldIssues.isEmpty { V15Section("需要修正") { ForEach(model.scheduleServerFieldIssues, id: \.code) { Text($0.message).font(V15Typography.secondary).foregroundStyle(V15Palette.danger.color) } }.accessibilityIdentifier("v15.f3b1.schedule.server-field-reasons") }
        if !model.scheduleServerReasons.isEmpty { V15Section("影响说明") { ForEach(model.scheduleServerReasons, id: \.self) { Text($0).font(V15Typography.secondary).foregroundStyle(V15Palette.ink.color.opacity(0.68)) } }.accessibilityIdentifier("v15.f3b1.schedule.server-reasons") }
    }
    ...
    @ViewBuilder private var previewSurface: some View {
        ...
        case .previewed:
            if let preview = model.schedulePreview {
                V15Section("第 3 步 · 确认影响", detail: "受影响账期 \(preview.affectedCycleCount) 个") {
                    ...
                    if let expectedAccountVersion = preview.expectedAccountVersion {
                        Text("预览基于当前账户数据")
                            .font(V15Typography.secondary)
                            .accessibilityIdentifier("v15.f3b1.schedule.preview.account-version.\(expectedAccountVersion)")
                    }
        ...
        case .unknown: VStack(alignment: .leading, spacing: V15Spacing.sm) {
            V15OutcomeUnknownState(message: "可以安全检查保存结果，或刷新账户后核对。")
