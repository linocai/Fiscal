...
public struct V15BootstrapView: View {
            ...
            VStack(alignment: .leading, spacing: 12) {
                if case .invalidAccessKey = model.phase {
                    Label("本机登录信息无效", systemImage: "key.slash")
                        .font(V15Typography.body.weight(.semibold)).foregroundStyle(V15Palette.danger.color)
                ...
                .background(passphraseFieldBackground, in: RoundedRectangle(cornerRadius: passphraseFieldCornerRadius, style: .continuous))
                .overlay { RoundedRectangle(cornerRadius: passphraseFieldCornerRadius, style: .continuous).stroke(passphraseBorderColor, lineWidth: passphraseHasError ? 1.5 : 1) }
    ...
    private var serviceFootnote: some View {
        ...
        .background(serviceFootnoteBackground, in: RoundedRectangle(cornerRadius: serviceFootnoteCornerRadius, style: .continuous))
        .overlay { RoundedRectangle(cornerRadius: serviceFootnoteCornerRadius, style: .continuous).stroke(V15Palette.hairline.color) }
        .frame(maxWidth: 390)
    }
    ...
    private func compactState(title: String, message: String, retry: Bool) -> some View {
        ...
        .overlay { RoundedRectangle(cornerRadius: compactStateCornerRadius, style: .continuous).stroke(compactStateBorderColor) }
        .padding(.top, 18)
    }

    private var passphraseHasError: Bool {
        switch model.phase {
        case .wrongPassphrase, .invalidAccessKey, .credentialGenerationChanged: true
        default: false
        }
    }
    private var passphraseBorderColor: Color {
#if os(iOS)
        passphraseHasError ? V15Palette.danger.color : V15Palette.hairline.color
#else
...
#if os(iOS)
        compactStateColor.opacity(0.45)
#else
        V15Palette.hairline.color
#endif
    }
    private var compactStateCornerRadius: CGFloat {
#if os(iOS)
        V15Radius.control
#else
        10
    ...
    private var bootstrapBackgroundColor: Color {
#if os(iOS)
        V15Palette.canvas.color
#else
        V15Palette.paper.color
#endif
    }
    private var passphraseFieldBackground: Color {
#if os(iOS)
    ...
    private var passphraseFieldCornerRadius: CGFloat {
#if os(iOS)
        V15Radius.control
#else
        10
    ...
    private var serviceFootnoteBackground: Color {
#if os(iOS)
        V15Palette.surfaceRaised.color
#else
        V15Palette.paper.color
    ...
    private var serviceFootnoteCornerRadius: CGFloat {
#if os(iOS)
        V15IOSLayout.cardCornerRadius
#else
        8
#endif
    }
    private var bootstrapOuterPadding: CGFloat {
#if os(iOS)
        V15IOSLayout.contentPadding
#else
        28
#endif
    }
    private var bootstrapCardPadding: CGFloat {
#if os(iOS)
        V15Spacing.lg
#else
        0
