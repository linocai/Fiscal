...
actor F4ATransport: V15Transporting {
    ...
    func send<Response: Decodable & Sendable>(_ request: V15Request, body: JSONValue?) async throws -> Response {
        ...
        let report = V15F4AFixtures.report(period: period, kind: isYear ? "year" : "month", revision: revision, empty: mode == .empty, completenessOnly: mode == .completenessOnly, unknown: mode == .unknown, unknownAccount: mode == .unknownAccount)
