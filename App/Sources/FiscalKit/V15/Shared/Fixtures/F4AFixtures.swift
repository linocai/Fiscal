...
public enum V15F4AFixtures {
    ...
    static func report(period: String = "2026-08", kind: String = "month", revision: Int64 = 77, empty: Bool = false, summaryOnly: Bool = false, completenessOnly: Bool = false, unknown: Bool = false, unknownAccount: Bool = false) -> String {
        ...
        let dateFrom = kind == "year" ? "\(period)-01-01" : "\(period)-01"
        let dateTo = kind == "year" ? "\(period)-12-31" : "\(period)-31"
        let completeness = empty ? "0,\"failed_import_count\":0,\"uncategorized_transaction_count\":0" : completenessOnly ? "1,\"failed_import_count\":2,\"uncategorized_transaction_count\":3" : "1,\"failed_import_count\":0,\"uncategorized_transaction_count\":1"
        let summary = empty
