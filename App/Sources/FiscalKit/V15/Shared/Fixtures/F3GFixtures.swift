...
public enum V15F3GFixtures {
    public static let snapshotID = UUID(uuidString: "00000000-0000-0000-0000-000000007312")!
    static func providerResponse(status: String = "succeeded", replay: Bool = false) -> String {
        "\(batch(status: status == "started" ? "parsing" : "review_required", version: 4).dropLast()),\"provider_attempt_id\":\"\(providerID)\",\"attempt_id\":\"\(attemptID)\",\"provider_snapshot_id\":\(status == "succeeded" ? "\"\(snapshotID)\"" : "null"),\"provider\":\"synthetic_statement\",\"provider_model\":\"synthetic-statement-v1\",\"prompt_version\":\"statement-p26-v1\",\"schema_version\":\"statement-provider-v1\",\"provider_status\":\"\(status)\",\"candidate_count\":1,\"replay\":\(replay),\"execution_scope\":\"request_bound\"}"
    }
    static func authorizationPreview() -> String {
        "{\"batch_version\":3,\"configured\":true,\"provider\":\"synthetic_statement\",\"provider_model\":\"synthetic-statement-v1\",\"prompt_version\":\"statement-p26-v1\",\"schema_version\":\"statement-provider-v1\",\"evidence_sha256\":\"\(String(repeating: "b", count: 64))\",\"page_numbers\":[1],\"row_count\":1,\"redaction_count\":0,\"redaction_version\":\"statement-redaction-v1\",\"description\":\"合成测试授权\"}"
    }
...
actor F3GTransport: V15Transporting {
    ...
    func send<Response: Decodable & Sendable>(_ request: V15Request, body: JSONValue?) async throws -> Response {
        ...
        if request.method == "GET", request.path.contains("/provider-attempts/by-idempotency/") {
            return try decode(V15F3GFixtures.providerResponse(status: providerCount == 1 ? "started" : "succeeded", replay: true))
        }
        switch (request.path, request.method) {
        case ("statement-imports/\(V15F3GFixtures.batchID)/provider-authorization", "GET"):
            return try decode(V15F3GFixtures.authorizationPreview())
        case ("statement-imports/\(V15F3GFixtures.batchID)/recovery", "GET"):
            return try decode("{\"batch\":\(V15F3GFixtures.batch()),\"next_action\":\"review\",\"row_count\":1,\"has_confirmed_rows\":false}")
        ...
        case ("statement-imports/\(V15F3GFixtures.batchID)/provider-attempts", "POST"):
            providerCount += 1; providerWireStarted = true; if mode == .delayedProvider && providerCount == 1 { try await Task.sleep(for: .seconds(2)) }; if mode == .providerUnknown && providerCount == 1 { throw V15Failure(kind: .responseUnknown, message: "解析响应未知。") }; return try decode(V15F3GFixtures.providerResponse(replay: providerCount > 1))
        ...
        case ("statement-imports/\(V15F3GFixtures.batchID)/review-workbench", "GET"):
            ...
            var board = V15F3GFixtures.workbench(route: route, cursor: cursor, evidenceState: filterObject["evidence_state"] as? String)
            if mode == .multipleMatches {
                board = board.replacingOccurrences(of: "\"candidates\":[", with: "\"candidates\":[{\"id\":\"00000000-0000-0000-0000-000000007320\",\"candidate_kind\":\"existing_transaction\",\"transaction_id\":\"00000000-0000-0000-0000-000000007321\",\"transaction_date\":\"2026-08-15\",\"amount_minor\":1850},")
            }
            return try decode(board)
        ...
        case ("statement-imports/\(V15F3GFixtures.batchID)/confirm", "POST"):
            confirmCount += 1; confirmWireStarted = true; if mode == .delayedConfirm && confirmCount == 1 { try await Task.sleep(for: .seconds(2)) }; if mode == .confirmConflict || (mode == .confirmationRejectedAfterUnknown && confirmCount > 1) { throw V15Failure(kind: .conflict, code: "resource_version_conflict", message: "行版本已变化。") }; if (mode == .unknown || mode == .confirmationNotArrived || mode == .confirmationRejectedAfterUnknown || mode == .confirmationRecoveryUnauthorized || mode == .confirmationRecoveryRateLimited) && confirmCount == 1 { throw V15Failure(kind: .responseUnknown, message: "确认响应未知。") }; if confirmCount == 2 && (mode == .confirmationRecoveryUnauthorized || mode == .confirmationRecoveryRateLimited) { throw V15Failure(kind: .transport, code: mode == .confirmationRecoveryUnauthorized ? "unauthorized" : "rate_limited", message: "恢复请求被网关拒绝", isDefinitiveRejection: true) }; return try decode(V15F3GFixtures.receipt(partial: mode == .partial))
        case ("statement-imports/\(V15F3GFixtures.batchID)/confirmation-receipt", "GET"):
            if (mode == .confirmationNotArrived || mode == .confirmationRejectedAfterUnknown || mode == .confirmationRecoveryUnauthorized || mode == .confirmationRecoveryRateLimited) && confirmCount == 1 { throw V15Failure(kind: .transport, code: "statement_import_confirmation_receipt_not_found", message: "Receipt missing") }
