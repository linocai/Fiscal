...
> 2026-09-12 立项并完成｜状态：R1–R5 修复及独立复查通过，2.3.0（43）已发布｜主入口：[PROJECT_PLAN.md](../../../PROJECT_PLAN.md)
...
不增加自动预测、预算、利息计算或新借款产品体系。用户取消了真实借款录入；全部验证使用合成数据，不创建生产账户/账目。实施阶段未部署后端或替换客户端；现已授权完整发布，进度与结果见 [RELEASE_STATE.md](RELEASE_STATE.md)。不生成 IPA。已发布 2.2.1 的事实保持在其原发布记录。
...
最终QA补充固定提交 `92a630778ae2c0e5cd231232499dd054d6b5c86e` 亦经原独立 reviewer 只读核对通过：隔离只读桥接不进入正式App，回执JSON与真实HTTP输出hash一致，截图与金额断言相符，未掩盖长流程验收限制。**累计已审修复范围 `3f79bdb` → `92a6307`，全部问题关闭；本节最后的记录提交不改变已审代码。**

## 13. 一条龙发布闭环

2026-09-12 用户授权后，冻结已审源码 `fa4181f` / `v2.3.0-build43`，完成双端签名构建、NB 0039→0040前后备份与迁移/恢复验证、Mac备份换装及真实只读验收，iOS Xcode就绪。最终事实、校验和及恢复路径只在 [RELEASE_STATE.md](RELEASE_STATE.md) 维护。

## 14. 2026-09-13 build44 结清后新消费转分期快修

用户反馈现有消费转分期被“账期已有还款”拦截。只读核对生产确认：同一未出账账期中，旧分期结清先发生，新消费后发生。此前将整个账期的历史还款当作所有消费的锁定条件，误伤结清后的新消费；客户端又在资格返回`eligible=false`后继续请求账期选项，将真实业务否决覆盖成英文读取失败。

本次按快修实施，营销版本保持2.3.0，build递增至44。实施基线`153489367cdef7d4829050bf03b453def8b89164`；生产源仍为`fa4181f02025da25b15053eee12075ffe1a84bd4`，两者原App/Backend差异为空。本轮范围为当前工作区App/Backend修复及新增专项测试，准确文件与指纹见[验证记录](qa/build44/verification.json)。六个预存用户scheme及上一轮清理脚本/文档不属于本轮产品修复审查范围。

- 资格判断、期间锁定、修改后缀统一核对未作废还款/本金减免/费用返还的业务发生时间。仅发生在消费之前的旧还款不会锁新消费；消费同刻或之后出现偿还仍保守拒绝整笔转分期，不猜测已还本金的逐笔分配。写入继续持有既有mutation lock并运行账期及账户的逐时点负债不变量校验。
- 客户端直接复用资格响应自带的账期选项，移除重复请求；iOS/macOS共用中文原因，业务否决正常显示“当前不能分期”。无新增下拉按钮或布局重做。
- 专项隔离PG用例先在原逻辑重现失败，修复后5项通过：同账期与次月起始的12期创建、期数修改、再次结清及撤销、旧分期/旧还款不改写；后续部分/全额/同刻还款拒绝，作废后重新允许。合成本金500400分，12期各41700分，总额与账户欠款核对。
- 最终Backend全量474项通过；Ruff检查/格式、Pyright零错误；FiscalKit 44 suites执行成功：xcresult统计459项通过、1项PDF/loopback联调因未设置`FISCAL_STATEMENT_E2E_URL`按原条件跳过；运行器日志将总计显示为460项，补充业务否决不再请求账期选项的断言，fixture按真实后端行为拒绝不可分期的选项请求。原有FastAPI/Starlette弃用警告保留；Backend无跳过。
...
## 16. 2026-09-22 build47 既有免息贷款账期修复

- 用户明确快修；确认免息、每期2533元，2026-09-22工商3495现实已付款，付款后36期本金91188元。现存37期现金流是计划，不是负债；旧车贷账户期初5066元已还清。不得伪造消费或到账，不改写旧期初和历史余额。
- 实施基线128475d（已发布客户端b4848d3/build46），后端实际3b29dcd/0040。新增loan_principal系统记账类型及0041数据库形状约束；独立确认当前本金93721元，通过既有37期零手续费分期和首期正常还款得到91188元。恢复命令检查账户版本、零债务、完整逐期金额/日期/版本、幂等键，一次事务取消替代计划并留修订；默认dry-run且执行全部延迟约束。
- 验证：5项新隔离数据库测试通过，含本金/账期/还款/撤销、重复执行、输入和版本冲突、消费及现金流、历史月余额、归档解密。全量后端首次473通过、6项既有固定日期测试失败；限定测试时钟后，涉及模块与新增用例16项全部通过，生产逾期保护不变。Ruff、格式、Pyright通过；双端Debug App与三组Release App构建通过，严格验签与dSYM对应校验通过。本轮未新增Apple单测轮次，也未进行独立复审。
- 已发布db95c136ddfb8efeba8e30e3ca59a4dc6db25c25 / v2.3.0-build47 / 0041。迁移前后15张财务表指纹一致，前后备份与恢复验证通过。用户scheme逐字节保留；既有AGENTS与scheme修改不提交。
- 车贷恢复命令先dry-run、再一次事务apply，使用同一幂等键；复跑只返回同一回执，无重复入账。9月22日本金确认93721元与工商3495正常还款2533元已写入，剩余91188元、36期、下期2026-10-22。取消37条被替代手工计划并保留历史；旧期初5066元不变。此处是真实账目变更，不沿用迁移前后“数据不变”的结论。
