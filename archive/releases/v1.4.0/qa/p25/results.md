...
- `xcodebuild test -project App/Fiscal.xcodeproj -scheme FiscalmacOS -configuration Debug -derivedDataPath /tmp/fiscal-p25-test-derived CODE_SIGNING_ALLOWED=NO` → **107 tests / 20 suites passed**。P25 合成夹具覆盖文本多页页序/坐标、扫描、混合区域去重、空白、加密、旋转、文件/页数/像素/OCR 字符上限，以及三条临时清理路径。
- `xcodebuild build -project App/Fiscal.xcodeproj -scheme FiscalmacOS -configuration Debug -derivedDataPath /tmp/fiscal-p25-macos-derived CODE_SIGNING_ALLOWED=NO` → **BUILD SUCCEEDED**。
- `xcodebuild build -project App/Fiscal.xcodeproj -scheme FiscaliOS -configuration Debug -sdk iphonesimulator -destination 'generic/platform=iOS Simulator' -derivedDataPath /tmp/fiscal-p25-ios-derived CODE_SIGNING_ALLOWED=NO` → **BUILD SUCCEEDED**。
