...
日期：2026-07-18  
状态：代码、静态检查、签名构建、生产部署和 Mac 安装通过；实体 iPhone 启动验收待完成。
...
| P7/P17/P18 PostgreSQL 定向报告组 | 13 passed；含平账根分类/子分类、消费/现金流合计及两种下钻排除 |
...
应用回退使用 `/Applications/Fiscal-build11-backup.app`；仅在数据库 Alembic head 与目标 revision 一致时可走既有 `Backend/ops/production/scripts/rollback.sh`。否则按生产 runbook 从已验证备份恢复到新库，禁止盲目 downgrade。
