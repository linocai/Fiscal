...
全仓 grep 收口：`App/` 生产代码与文案中 `设备密钥`/`一次性`/`粘贴`/`配对`/`fiscal://`/`PairingLink`
...
口令只经 stdin、不落参数/日志。步骤见 `Backend/ops/production/README.md` §Access passphrase。
