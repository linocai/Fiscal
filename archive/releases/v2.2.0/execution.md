...
> 2026-09-10 从根 PROJECT_PLAN.md 移入；以下是已结束版本的历史契约与证据，不再维护当前状态。现行入口为 [PROJECT_PLAN.md](../../../PROJECT_PLAN.md)。旧文中的发布授权仅属于该历史版本。
...
- 用户录屏反馈：整体方向接受；修复总览拖宽窗口出现的白色空带，Mac 字体轻微放大，“设置与数据 / 财务分析 / 记一笔”统一现代按钮样式。只增 Build 到 41，营销版本保留 2.2.0；不重做页面结构、不改账本逻辑。
