...
public struct InstallmentCreateSheet: View {
    ...
    public var body: some View {
        NavigationStack {
            Form {
                Section("关联信用消费") { LabeledContent("消费", value: purchase.title); LabeledContent("本金", value: Money(minorUnits: purchase.amountMinor).formatted()) }
                Section("分期安排") {
                ...
                if let eligibility = installments.eligibility, !eligibility.eligible { Section { Label(eligibility.reasonCode ?? "该消费当前不能分期", systemImage: "exclamationmark.triangle").foregroundStyle(FiscalColor.expense) } }
                if optionsLoading { Section { ProgressView("正在读取分类…") } }
                if let optionsError { Section { Label(optionsError, systemImage: "wifi.exclamationmark").foregroundStyle(FiscalColor.expense); Button("重试") { Task { await loadDependencies() } } } }
                if let message = validation ?? installments.message { Section { Label(message, systemImage: "exclamationmark.triangle").foregroundStyle(FiscalColor.expense) } }
                Section { Text("分期只安排已记入账本的本金与固定手续费；不会再次记录消费。未来计划额是未扣除通用部分还款的毛额。") }.font(.caption).foregroundStyle(FiscalColor.tertiary)
            }
            .navigationTitle("创建分期计划")
            .toolbar { ToolbarItem(placement: .cancellationAction) { Button("取消") { dismiss() } }; ToolbarItem(placement: .confirmationAction) { Button("创建") { create() }.disabled(installments.isMutating || installments.eligibility?.eligible != true || optionsLoading || optionsError != nil) } }
        ...
        NavigationStack {
            Form {
                ...
                if let preview = installments.changePreview {
                    Section("服务器影响预览") { LabeledContent("未来计划毛额", value: Money(minorUnits: preview.proposedPlan.futureScheduledGrossMinor).formatted()); LabeledContent("受影响账期", value: "\(preview.affectedCycles.count) 个"); ForEach(preview.warnings) { Label($0.message, systemImage: "exclamationmark.triangle") } }
                }
                if optionsLoading { Section { ProgressView("正在读取账户、分类与账期…") } }
                if let optionsError { Section { Label(optionsError, systemImage: "wifi.exclamationmark").foregroundStyle(FiscalColor.expense); Button("重试") { Task { await loadDependencies() } } } }
                if installments.conflictDetected { Section { Label("计划已在其他操作中变化。", systemImage: "arrow.triangle.2.circlepath").foregroundStyle(FiscalColor.expense); Button("刷新计划") { refreshConflict() } } }
                if let message = validation ?? installments.message { Section { Label(message, systemImage: "exclamationmark.triangle").foregroundStyle(FiscalColor.expense) } }
            }
            .navigationTitle("编辑分期")
            .toolbar { ToolbarItem(placement: .cancellationAction) { Button("取消") { dismiss() } }; ToolbarItem(placement: .confirmationAction) { if installments.changePreview == nil { Button("预览") { preview() }.disabled(optionsLoading || optionsError != nil) } else { Button("确认保存") { save() }.disabled(installments.isMutating || optionsLoading || optionsError != nil) } } }
            .task { await loadDependencies() }
...
public struct InstallmentSettlementSheet: View {
    ...
    public var body: some View {
        NavigationStack { Form {
            Section("提前结清") { Picker("付款账户", selection: $paymentAccountID) { Text("请选择").tag(Optional<UUID>.none); ForEach(accountOptions.filter { $0.kind != .credit && $0.archivedAt == nil }) { Text($0.name).tag(Optional($0.id)) } }; Picker("目标账单日", selection: $targetStatementDate) { Text("请选择").tag(""); ForEach(installments.cycleOptions.filter(\.eligible)) { Text("\($0.statementDate) · 到期 \($0.dueDate)").tag($0.statementDate) } }; DatePicker("还款时间", selection: $occurredAt) }
            if let value = installments.settlementPreview { Section("服务器确认") { LabeledContent("实际还款", value: Money(minorUnits: value.amountMinor).formatted()); LabeledContent("还款后信用负债", value: Money(minorUnits: value.debtAfterMinor).formatted()); ForEach(value.warnings) { Text($0.message) } } }
            if optionsLoading { Section { ProgressView("正在读取付款账户与账期…") } }
            if let optionsError { Section { Label(optionsError, systemImage: "wifi.exclamationmark").foregroundStyle(FiscalColor.expense); Button("重试") { Task { await loadDependencies() } } } }
            if installments.conflictDetected { Section { Label("计划版本已变化。", systemImage: "arrow.triangle.2.circlepath").foregroundStyle(FiscalColor.expense); Button("刷新计划") { refreshConflict() } } }
            if let message = installments.message { Section { Label(message, systemImage: "exclamationmark.triangle").foregroundStyle(FiscalColor.expense) } }
        }.navigationTitle("提前结清").toolbar { ToolbarItem(placement: .cancellationAction) { Button("取消") { dismiss() } }; ToolbarItem(placement: .confirmationAction) { if installments.settlementPreview == nil { Button("预览影响") { preview() }.disabled(optionsLoading || optionsError != nil) } else { Button("确认还款") { settle() }.disabled(installments.isMutating || optionsLoading || optionsError != nil) } } }.task { await loadDependencies() }.onChange(of: paymentAccountID) { _, _ in installments.invalidateSettlementPreview() }.onChange(of: targetStatementDate) { _, _ in installments.invalidateSettlementPreview() }.onChange(of: occurredAt) { _, _ in installments.invalidateSettlementPreview() } }.installmentEditorFrame(width: 500, height: 540)
    }
...
public struct InstallmentCancellationSheet: View {
    ...
    public var body: some View { NavigationStack { Form { Section { DatePicker("退款确认时间", selection: $occurredAt); Text("仅取消未锁定的完整未来期次，并生成真实退款。此操作不可恢复。") }.foregroundStyle(FiscalColor.secondary); if let value = installments.cancellationPreview { Section("服务器确认") { LabeledContent("本金退款", value: Money(minorUnits: value.principalRefundMinor).formatted()); LabeledContent("手续费退款", value: Money(minorUnits: value.feeRefundMinor).formatted()); LabeledContent("取消期数", value: "\(value.cancelledPeriods.count) 期"); LabeledContent("退款后信用负债", value: Money(minorUnits: value.debtAfterMinor).formatted()) } }; if installments.conflictDetected { Section { Label("计划版本已变化。", systemImage: "arrow.triangle.2.circlepath").foregroundStyle(FiscalColor.expense); Button("刷新计划") { refreshConflict() } } }; if let message = installments.message { Section { Label(message, systemImage: "exclamationmark.triangle").foregroundStyle(FiscalColor.expense) } } }.navigationTitle("取消未来期次").toolbar { ToolbarItem(placement: .cancellationAction) { Button("取消") { dismiss() } }; ToolbarItem(placement: .confirmationAction) { if installments.cancellationPreview == nil { Button("预览退款") { preview() } } else { Button("确认取消", role: .destructive) { cancel() }.disabled(installments.isMutating) } } }.onChange(of: occurredAt) { _, _ in installments.invalidateCancellationPreview() } }.installmentEditorFrame(width: 480, height: 460) }
...
public struct InstallmentReverseSettlementSheet: View {
    ...
    @State private var occurredAt = Date(); @State private var idempotencyKey = UUID()
    public init(installments: InstallmentModel, plan: InstallmentPlanDTO) { self.installments = installments; self.plan = plan }
    public var body: some View { NavigationStack { Form { Section { DatePicker("撤销时间", selection: $occurredAt); Text("只在后续没有依赖这笔结清还款时可撤销。服务器会恢复原期次与账期。") }.foregroundStyle(FiscalColor.secondary); if let value = installments.reversePreview { Section("服务器确认") { LabeledContent("可撤销", value: value.eligible ? "是" : "否"); LabeledContent("撤销后信用负债", value: Money(minorUnits: value.debtAfterMinor).formatted()); LabeledContent("恢复期次", value: "\(value.restoredPeriods.count) 期") } }; if installments.conflictDetected { Section { Label("计划版本已变化。", systemImage: "arrow.triangle.2.circlepath").foregroundStyle(FiscalColor.expense); Button("刷新计划") { refreshConflict() } } }; if let message = installments.message { Section { Label(message, systemImage: "exclamationmark.triangle").foregroundStyle(FiscalColor.expense) } } }.navigationTitle("撤销提前结清").toolbar { ToolbarItem(placement: .cancellationAction) { Button("取消") { dismiss() } }; ToolbarItem(placement: .confirmationAction) { if installments.reversePreview == nil { Button("预览影响") { preview() } } else { Button("确认撤销", role: .destructive) { reverse() }.disabled(installments.reversePreview?.eligible != true || installments.isMutating) } } }.onChange(of: occurredAt) { _, _ in installments.invalidateReversePreview() } }.installmentEditorFrame(width: 480, height: 440) }
    private var request: InstallmentOperationRequest { .init(expectedVersion: plan.version, occurredAt: occurredAt) }
