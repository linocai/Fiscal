...
public struct IOSCreditAccountsScreen: View {
    ...
    public init(credit: CreditModel, transactions: TransactionsModel, accounts: AccountsModel, categories: CategoriesModel) { self.credit = credit; self.transactions = transactions; self.accounts = accounts; self.categories = categories }
    public var body: some View {
        Group {
            ...
            case .loaded:
                ScrollView { LazyVStack(spacing: 14) { ForEach(credit.accounts) { summary in NavigationLink { IOSCreditAccountDetail(credit: credit, accountID: summary.accountID, transactions: transactions, accounts: accounts, categories: categories) } label: { accountCard(summary) }.buttonStyle(.plain) }; if !archivedCreditAccounts.isEmpty { Text("已归档信用账户").font(.headline).frame(maxWidth: .infinity, alignment: .leading).padding(.top, 8); ForEach(archivedCreditAccounts) { account in NavigationLink { IOSCreditAccountDetail(credit: credit, accountID: account.id, transactions: transactions, accounts: accounts, categories: categories, readOnly: true) } label: { archivedAccountCard(account) }.buttonStyle(.plain) } } }.padding(16).padding(.bottom, 90) }.background(FiscalColor.iOSBackground)
            }
        }
        .navigationTitle("信用账期").onAppear { Task { await reload() } }.refreshable { await reload() }
    }
    ...
    private var archivedOnlyList: some View { ScrollView { LazyVStack(alignment: .leading, spacing: 14) { Text("已归档信用账户").font(.headline); ForEach(archivedCreditAccounts) { account in NavigationLink { IOSCreditAccountDetail(credit: credit, accountID: account.id, transactions: transactions, accounts: accounts, categories: categories, readOnly: true) } label: { archivedAccountCard(account) }.buttonStyle(.plain) } }.padding(16).padding(.bottom, 90) }.background(FiscalColor.iOSBackground) }
...
public struct MacAccountsCreditScreen: View {
    ...
    @State private var repayCycle: CreditCycleDTO?
    @State private var archivedCreditAccounts: [AccountDTO] = []
    public init(accounts: AccountsModel, credit: CreditModel, transactions: TransactionsModel, categories: CategoriesModel) { self.accounts = accounts; self.credit = credit; self.transactions = transactions; self.categories = categories }
    public var body: some View {
        VStack(spacing: 0) {
    ...
    private func accountCard(_ account: AccountDTO) -> some View {
        Button {
            ...
            Task { await credit.loadAccount(account.id) }
        } label: {
    ...
    private func reload() async {
        ...
        if let selectedCreditID { await credit.loadAccount(selectedCreditID) }
    }
