...
public struct IOSCreditAccountDetail: View {
    ...
    @ViewBuilder private var installmentCards: some View {
        if installments.loadedAccountID == accountID, let error = installmentError {
            ...
            VStack(alignment: .leading, spacing: 10) { Text("分期计划").font(.headline); ForEach(installments.plans) { plan in NavigationLink { IOSInstallmentPlanDetail(installments: installments, planID: plan.id, accounts: accounts, categories: categories, readOnly: readOnly) } label: { VStack(alignment: .leading, spacing: 8) { HStack { Text(plan.title).font(.headline).lineLimit(2); Spacer(); Text("\(plan.cycleSettledCount + plan.cancelledCount) / \(plan.installmentCount) 期").font(.caption).foregroundStyle(FiscalColor.tertiary); Image(systemName: "chevron.right").font(.caption).accessibilityHidden(true) }; if let next = plan.nextPeriod { Text("下一期计划毛额 \(Money(minorUnits: next.amountDueMinor).formatted()) · 到期 \(next.dueDate)").font(.subheadline).fixedSize(horizontal: false, vertical: true) }; ProgressView(value: Double(plan.cycleSettledCount + plan.cancelledCount), total: Double(max(1, plan.installmentCount))).tint(FiscalColor.debt); HStack { Text("融资总额 \(Money(minorUnits: plan.totalFinancedMinor).formatted())"); Spacer(); Text("未来计划毛额 \(Money(minorUnits: plan.futureScheduledGrossMinor).formatted())") }.font(.caption).foregroundStyle(FiscalColor.secondary); Text("毛额未扣除通用部分还款；精确余额以账期为准。") .font(.caption2).foregroundStyle(FiscalColor.tertiary) }.padding(16).background(FiscalColor.surface, in: .rect(cornerRadius: 18)) }.buttonStyle(.plain) } }
        }
    }
...
public struct MacAccountsCreditScreen: View {
    ...
    private func accountCard(_ account: AccountDTO) -> some View {
        Button {
            ...
            .background(selectedCreditID == account.id ? FiscalColor.accent.opacity(0.10) : FiscalColor.surface, in: .rect(cornerRadius: 14))
        }
        .buttonStyle(.plain)
    }
