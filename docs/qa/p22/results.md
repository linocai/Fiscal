...
状态：**Automated Verified（本地）**；生产 Archive 隔离演练发现 fresh migrated target 的 `ai_settings` seed 契约缺口，已在本地修复并待新 shadow 复验；**尚未迁移/部署生产，macOS + Kurisu 物理验收未执行**。不打 tag/push。
...
- Fresh PostgreSQL `fiscal_p22_fresh`：`pytest --junitxml=/private/tmp/fiscal-p22-seed-fresh.xml`，JUnit `248 tests / 0 failures / 0 errors / 0 skipped`。
