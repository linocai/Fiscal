...
- Apple seam：`xcodebuild test -project apple/Fiscal.xcodeproj -scheme FiscalmacOS -configuration Debug -derivedDataPath /tmp/fiscal-p26-test-derived CODE_SIGNING_ALLOWED=NO` → **108 tests / 20 suites passed**；macOS 与 iOS Simulator Debug build 均 **BUILD SUCCEEDED**。
