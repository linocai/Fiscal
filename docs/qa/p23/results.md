...
- 用户已授权 P23 生产备份、0022→0023 隔离影子/财务指纹、生产迁移部署、可清理 AI 提案/质量事件 QA 和 Mac/Kurisu Build 21 验收。真实候选模型的 Provider shadow corpus 本次明确跳过，不记录为已验证。
- 用户同时明确取消该个人自用 App 的 7 天 Observed Stable 门；本期生产/恢复/双端/最终对账全绿后，当晚可直接创建 `v1.3.0` tag 并 push `main + tag`。
