...
async def test_regular_repayment_persists_completed_and_void_reopens_plan(
    session: AsyncSession,
) -> None:
    account, _purchase, plan = await seeded_plan(session)
    payment = await AccountService(session).create(
        ...
        repayments.append(
            await ledger.create(
                TransactionDraft(
                    ...
                    occurred_at=datetime(2027, 3, 1, 1, tzinfo=UTC),
                    title=f"常规结清第 {period.sequence} 期",
                    account_id=payment.id,
                    destination_account_id=account.id,
    ...
    completed = await InstallmentService(session).get(plan.id)
    assert completed.status is InstallmentPlanStatus.COMPLETED
    persisted = await InstallmentService(session).repository.plan(plan.id)
    assert persisted is not None and persisted.lifecycle == "completed"

    await ledger.void(repayments[-1].id, repayments[-1].version)
    reopened = await InstallmentService(session).get(plan.id)
    assert reopened.status is InstallmentPlanStatus.ACTIVE
    persisted = await InstallmentService(session).repository.plan(plan.id)
    assert persisted is not None and persisted.lifecycle == "active"


async def test_completed_plan_archive_payload_keeps_explicit_lifecycle(
    session: AsyncSession,
) -> None:
    account, _purchase, plan = await seeded_plan(session)
    payment = await AccountService(session).create(
    ...
    for period in plan.periods:
        await ledger.create(
            TransactionDraft(
                ...
                amount_minor=period.amount_due_minor,
                occurred_at=datetime(2027, 3, 1, 1, tzinfo=UTC),
                title=f"归档结清第 {period.sequence} 期",
                account_id=payment.id,
                destination_account_id=account.id,
                credit_cycle_id=period.effective_cycle_id,
            ),
    ...
    _manifest, payload = ArchiveService.open(archive, password=password)
    exported = next(
        item for item in payload["entities"]["installment_plans"] if item["id"] == str(plan.id)
    )
    assert exported["lifecycle"] == "completed"
