...
async def test_atomic_failure_rolls_back_purchase_and_schedule_change_rebinds(
    ...
    # This schedule-change scenario is about rebinding an open cycle, not the
    # host clock. Keep its business date inside the fixture cycle so it does
    # not become an overdue-cycle test as calendar time advances.
    credit = CreditService(session, today=lambda: date(2026, 7, 16))
