...
async def test_account_crud_order_archive_restore_delete_and_conflicts(
    ...
    order_state = await service.order_state()
    ordered = await service.reorder([credit.id, debit.id], order_state.list_revision)
    assert [item.id for item in ordered] == [credit.id, debit.id]
    with pytest.raises(APIError) as incomplete_order:
        await service.reorder([credit.id], order_state.list_revision)
...
async def test_category_hierarchy_nested_list_reorder_and_safe_delete(
    ...
    order_state = await service.order_state(parent_id=None, direction=CategoryDirection.EXPENSE)
    reordered = await service.reorder(
        parent_id=None,
        ordered_ids=[travel.id, food.id],
        expected_list_revision=order_state.list_revision,
    )
    assert [item.id for item in reordered] == [travel.id, food.id]
    with pytest.raises(APIError) as stale_order:
        await service.reorder(
            parent_id=None,
            ordered_ids=[food.id, travel.id],
            expected_list_revision=order_state.list_revision,
        )
    assert_error(stale_order, "list_revision_conflict")
    with pytest.raises(APIError) as mixed_direction:
        await service.reorder(
            parent_id=None,
            ordered_ids=[travel.id, salary.id, food.id],
            expected_list_revision=order_state.list_revision,
        )
