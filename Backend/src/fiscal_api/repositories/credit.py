...
class CreditRepository:
    ...
    async def preview(self, preview_id: UUID) -> CreditScheduleChangePreview | None:
        return await self.session.get(CreditScheduleChangePreview, preview_id)

    def add_preview(self, preview: CreditScheduleChangePreview) -> None:
        self.session.add(preview)

    async def operation(self, idempotency_key: UUID) -> CreditScheduleChangeOperation | None:
        return await self.session.scalar(
            select(CreditScheduleChangeOperation).where(
                CreditScheduleChangeOperation.idempotency_key == idempotency_key
            )
        )

    def add_operation(self, operation: CreditScheduleChangeOperation) -> None:
        ...
        return list(
            (
                await self.session.scalars(
                    ...
                    .where(LedgerTransaction.credit_cycle_id.in_(cycle_ids))
                    .order_by(
                        LedgerTransaction.credit_cycle_id,
                        LedgerTransaction.occurred_at,
    ...
    async def purchase_amounts(self, transaction_ids: list[UUID]) -> dict[UUID, int]:
        if not transaction_ids:
            return {}
        rows = await self.session.execute(
            select(LedgerTransaction.id, func.sum(-Posting.amount_minor))
            .join(Posting, Posting.transaction_id == LedgerTransaction.id)
            .where(
        ...
        return {transaction_id: int(amount) for transaction_id, amount in rows}
        ...
        return list(
            (
                await self.session.scalars(
                    ...
                    .order_by(
                        InstallmentPeriod.scheduled_cycle_id,
                        InstallmentPeriod.effective_cycle_id,
                        InstallmentPeriod.plan_id,
                        InstallmentPeriod.sequence,
                        InstallmentPeriod.id,
                    )
                )
            ).all()
        )
        ...
        return list(
            (
                await self.session.scalars(
                    ...
                    .where(InstallmentPlan.start_cycle_id.in_(cycle_ids))
                    .order_by(
                        InstallmentPlan.start_cycle_id,
    ...
    async def ai_proposals_for_cycles(self, cycle_ids: list[UUID]) -> list[AIProposal]:
        ...
        return list(
            (
                await self.session.scalars(
                    select(AIProposal)
                    .where(AIProposal.credit_cycle_id.in_(cycle_ids))
                    .order_by(AIProposal.credit_cycle_id, AIProposal.created_at, AIProposal.id)
                )
            ).all()
        )

    async def statement_rows_for_cycles(self, cycle_ids: list[UUID]) -> list[StatementImportRow]:
        if not cycle_ids:
            return []
        return list(
            (
                await self.session.scalars(
                    ...
                    .where(StatementImportRow.credit_cycle_id_candidate.in_(cycle_ids))
                    .order_by(
                        StatementImportRow.credit_cycle_id_candidate,
                        StatementImportRow.created_at,
                        StatementImportRow.id,
                    )
                )
            ).all()
        )
    ...
    async def cycle_is_referenced(self, cycle_id: UUID) -> bool:
        ...
        if proposal is not None:
            return True
        statement_row = await self.session.scalar(
            select(StatementImportRow.id)
            .where(StatementImportRow.credit_cycle_id_candidate == cycle_id)
            .limit(1)
        )
