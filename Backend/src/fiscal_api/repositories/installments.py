...
class InstallmentRepository:
    ...
    async def plans_for_credit_accounts(
        self, account_ids: set[UUID], *, for_update: bool = False
    ) -> list[InstallmentPlan]:
        if not account_ids:
            return []
        statement = (
            select(InstallmentPlan)
            .where(InstallmentPlan.credit_account_id.in_(account_ids))
            .options(selectinload(InstallmentPlan.periods))
        )
        if for_update:
            statement = statement.with_for_update()
        ...
        *,
        ...
        if status == "cancelled":
            ...
            statement = statement.where(
                or_(
                    InstallmentPlan.lifecycle == "completed",
                    and_(
                        InstallmentPlan.lifecycle.in_(["active", "partially_cancelled"]),
                        completed,
                    ),
                )
            )
    ...
    async def linked_transaction_ids_for(self, transaction_ids: set[UUID]) -> set[UUID]:
        if not transaction_ids:
            return set()
        return set(
            (
                await self.session.scalars(
                    select(InstallmentLedgerLink.transaction_id).where(
                        InstallmentLedgerLink.transaction_id.in_(transaction_ids)
                    )
                )
            ).all()
        )
