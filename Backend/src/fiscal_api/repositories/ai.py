...
class AIRepository:
    ...
    async def latest_policy(self, *, for_update: bool = False) -> AIExecutionPolicy | None:
        statement = (
    ...
    async def latest_policy_for_scope(
        self,
        *,
        source: str | None,
        transaction_kind: str | None,
        for_update: bool = False,
    ) -> AIExecutionPolicy | None:
        statement = select(AIExecutionPolicy).where(
            AIExecutionPolicy.source.is_(None)
            if source is None
            else AIExecutionPolicy.source == source,
            AIExecutionPolicy.transaction_kind.is_(None)
            if transaction_kind is None
            else AIExecutionPolicy.transaction_kind == transaction_kind,
        )
