...
class ReimbursementRepository:
    ...
    async def transaction_relation_flags(
        self, transaction_ids: set[UUID]
    ) -> dict[UUID, tuple[bool, bool]]:
        """Return allocation and receipt existence without per-row relation loading."""
        if not transaction_ids:
            return {}
        allocation_ids = set(
            (
                await self.session.scalars(
                    select(ReimbursementAllocation.transaction_id).where(
                        ReimbursementAllocation.transaction_id.in_(transaction_ids)
                    )
                )
            ).all()
        )
            ...
            (
                await self.session.scalars(
                    select(ReimbursementReceipt.transaction_id).where(
                        ReimbursementReceipt.transaction_id.in_(transaction_ids)
                    )
                )
            ).all()
        )
        return {
            transaction_id: (transaction_id in allocation_ids, transaction_id in receipt_ids)
    ...
    async def expense_candidates_page(
        self,
        *,
        limit: int,
        query: str | None,
    ...
    async def candidate_financials(
        self, transaction_ids: set[UUID]
    ) -> tuple[dict[UUID, int], dict[UUID, int]]:
        """Return reimbursable capacity and effective allocations in fixed query count."""
        if not transaction_ids:
            return {}, {}
        capacity_rows = await self.session.execute(
            select(
                LedgerTransaction.id,
                func.fiscal_reimbursement_expense_capacity(LedgerTransaction.id),
            ).where(LedgerTransaction.id.in_(transaction_ids))
        )
        capacities = {
            transaction_id: int(capacity or 0) for transaction_id, capacity in capacity_rows
        }
        active_received = (
            select(func.coalesce(func.sum(ReimbursementReceiptAllocation.amount_minor), 0))
            .join(
        ...
        allocated_rows = await self.session.execute(
            select(
                ReimbursementAllocation.transaction_id,
                func.coalesce(func.sum(effective), 0),
            )
            .join(ReimbursementClaim, ReimbursementClaim.id == ReimbursementAllocation.claim_id)
            .where(ReimbursementAllocation.transaction_id.in_(transaction_ids))
            .group_by(ReimbursementAllocation.transaction_id)
        )
        allocated = {transaction_id: int(amount or 0) for transaction_id, amount in allocated_rows}
        return capacities, allocated
