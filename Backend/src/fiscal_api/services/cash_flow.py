...
class CashFlowService:
    ...
    async def history(self, month: str | None) -> CashFlowHistoryResponse:
        ...
        reimbursement_values = await self._reimbursement_values_by_party()
        items = [
        ...
        items.extend(
            self._system_override_response(
                ...
                planned_amount_minor=(
                    reimbursement_values[item.system_reference_id][1]
                    if item.system_reference_id in reimbursement_values
                    else 0
                    if item.system_kind == CashFlowSystemKind.REIMBURSEMENT.value
                    else item.planned_amount_minor or 0
                ),
            )
    ...
    async def update_system(
        ...
        if override is None:
            ...
            override.planned_amount_minor = None
        ...
        await self.session.commit()
        reimbursement_values = await self._reimbursement_values_by_party()
        return self._system_override_response(
            override,
            self._today(),
            planned_amount_minor=(
                reimbursement_values[reference_id][1] if reference_id in reimbursement_values else 0
            ),
        )
    ...
    async def _raw_system_items(
        ...
        party_values = {
            party_id: value
            for party_id, value in (await self._reimbursement_values_by_party()).items()
            if value[1] > 0
        }
    ...
    async def _reimbursement_values_by_party(
        self,
    ) -> dict[UUID, tuple[ReimbursementFact, int]]:
        ...
        return {party_id: (value[0], max(0, value[1])) for party_id, value in party_values.items()}
    ...
    async def _validate_references(self, draft: CashFlowDraft | CashFlowReplace) -> None:
    ...
    def _apply_draft(
        item: CashFlowItem,
        draft: CashFlowDraft | CashFlowReplace,
        *,
