...
_EXCLUDED_TABLES = {
    ...
    "access_keys",
    "data_revision",
    # Preview tokens and their operation receipts are short-lived operational state,
    # not financial facts and must never be restored into a new archive target.
...
_EXCLUDED_FOREIGN_KEY_COLUMNS = {
    ...
    # they consumed is deliberately not archive material.
    ("reimbursement_operations", "preview_id"),
}
_AI_SECRET_COLUMNS = {
...
def _retire_ai_auto_execute(table_name: str, values: Mapping[str, object]) -> dict[str, object]:
    """Fail closed for both archive export and every restore/import caller."""
    sanitized = dict(values)
    if table_name in {"ai_settings", "ai_execution_policies"}:
        sanitized["auto_execute_enabled"] = False
    return sanitized
        ...
        *,
        ...
        for table in tables:
            ...
            for raw in raw_rows:
                ...
                decoded = _retire_ai_auto_execute(
                    table.name,
                    {
