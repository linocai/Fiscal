...
class TransactionService:
    ...
    async def list(
        ...
        *,
        ...
        return TransactionPage(
            items=await self.responses_with_capabilities(page),
    ...
    async def update(
        ...
        check_version(
            ...
            reload_path=f"/api/v1/transactions/{transaction.id}",
        )
        if transaction.voided_at is not None:
        ...
        *,
        ...
        try:
            ...
            if credit_accounts:
                ...
                await InstallmentService(self.session).refresh_lifecycles_for_credit_accounts(
                    credit_accounts
                )
        except APIError:
    ...
    def record_external_update_revision(
        self, transaction: LedgerTransaction, response: TransactionResponse
    ) -> None:
        """Record a formal update performed by an atomic cross-domain command."""
        self._add_revision(transaction, RevisionEvent.UPDATED, response)
    ...
    def _response(
        ...
        *,
        has_installment_relation: bool = False,
        has_reimbursement_claim: bool = False,
        has_reimbursement_receipt: bool = False,
    ) -> TransactionResponse:
        ...
        return TransactionResponse(
            ...
            available_actions=TransactionService._available_actions(
                voided=transaction.voided_at is not None,
                source=transaction.source,
                has_installment_relation=has_installment_relation,
                has_reimbursement_claim=has_reimbursement_claim,
                has_reimbursement_receipt=has_reimbursement_receipt,
            ),
        )
    ...
    def snapshot_response(
        ...
        *,
        ...
        return TransactionService._response(
            transaction,
            postings,
            has_installment_relation=has_installment_relation,
            has_reimbursement_claim=has_reimbursement_claim,
    ...
    async def responses_with_capabilities(
        self, transactions: list[LedgerTransaction]
    ) -> list[TransactionResponse]:
        """Build list/overview rows with batched relation capability facts."""
        transaction_ids = {transaction.id for transaction in transactions}
        reimbursement_flags = await self.reimbursement_repository.transaction_relation_flags(
            transaction_ids
        )
        installment_ids = await self.installment_repository.linked_transaction_ids_for(
            transaction_ids
        )
        responses: list[TransactionResponse] = []
        for transaction in transactions:
            has_claim, has_receipt = reimbursement_flags.get(transaction.id, (False, False))
            responses.append(
                self._response(
                    transaction,
                    list(transaction.postings),
                    has_installment_relation=transaction.id in installment_ids,
                    has_reimbursement_claim=has_claim,
                    has_reimbursement_receipt=has_receipt,
                )
            )
        return responses
    ...
    async def response_with_relation(
            ...
            return response.model_copy(
                update={
                    "available_actions": self._available_actions(
                        voided=transaction.voided_at is not None,
                        source=transaction.source,
                        has_installment_relation=link is not None,
                        has_reimbursement_claim=False,
                        has_reimbursement_receipt=False,
                    )
                }
            )
        ...
        return response.model_copy(
            update={
                "reimbursement_relations": relations,
                "available_actions": self._available_actions(
                    voided=transaction.voided_at is not None,
                    source=transaction.source,
                    has_installment_relation=link is not None,
                    has_reimbursement_claim=bool(allocations),
                    has_reimbursement_receipt=receipt is not None,
                ),
            }
        )
    ...
    def _available_actions(
        *,
        voided: bool,
        source: str,
        has_installment_relation: bool,
        has_reimbursement_claim: bool,
        has_reimbursement_receipt: bool,
    ) -> list[AvailableAction]:
        """Expose the server's void eligibility without duplicating it in clients."""
        reason_code: str | None = None
        reason_message: str | None = None
        if has_reimbursement_receipt:
            reason_code = "reimbursement_receipt_in_use"
            reason_message = "Use the reimbursement receipt command to change this transaction"
        elif has_reimbursement_claim:
            reason_code = "reimbursement_claim_in_use"
            reason_message = "The source expense is used by a reimbursement claim"
        elif source == TransactionSource.SYSTEM.value or has_installment_relation:
            reason_code = "installment_plan_in_use"
            reason_message = "Use the installment plan command to change this transaction"
        elif voided:
