...
from fiscal_api.api.p3_schemas import AvailableAction, PostingResponse, TransactionResponse
...
class InstallmentService:
    ...
    async def refresh_lifecycles_for_credit_accounts(self, account_ids: set[UUID]) -> None:
        """Persist the derived completed state at the same write boundary.

        Repayments are ledger writes and can settle (or, when voided/rebound,
        reopen) an installment plan without going through an installment route.
        Keeping this transition here makes the stored lifecycle, API status,
        Archive payload and future-event eligibility agree in one transaction.
        """
        for plan in await self.repository.plans_for_credit_accounts(account_ids, for_update=True):
            if plan.lifecycle in {"cancelled", "settled_early"}:
                continue
            response = await self.response(plan)
            active_periods = [item for item in response.periods if item.cancelled_at is None]
            complete = bool(active_periods) and all(
                item.cycle_status is CreditCycleStatus.SETTLED for item in active_periods
            )
            fallback = (
                "partially_cancelled"
                if any(item.cancelled_at is not None for item in response.periods)
                else "active"
            )
            desired = "completed" if complete else fallback
            if plan.lifecycle == desired:
                continue
            plan.lifecycle = desired
            plan.version += 1
            plan.updated_at = utc_now()
            await self.session.flush()
            snapshot = await self.response(plan)
            self.repository.add_revision(
                InstallmentPlanRevision(
                    plan_id=plan.id,
                    version=plan.version,
                    event="completed" if desired == "completed" else "reopened",
    ...
    async def response(self, plan: InstallmentPlan) -> InstallmentPlanResponse:
        ...
        completed = active and all(
            item.cycle_status is CreditCycleStatus.SETTLED for item in active
        )
        status = (
            lifecycle_status
            if lifecycle_status
            ...
            if lifecycle_status is InstallmentPlanStatus.COMPLETED
            and any(item.cancelled_at is not None for item in periods)
