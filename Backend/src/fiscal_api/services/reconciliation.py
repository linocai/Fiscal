...
from fiscal_api.services.common import acquire_mutation_lock, invalid, not_found


class ReconciliationService:
    """Derived reconciliation read-model. It cannot write ledger or postings."""

    _DISMISSIBLE_ATTENTION_TYPES: ClassVar[set[str]] = {
    ...
    _STATEMENT_IMPORT_ATTENTION_TYPES: ClassVar[set[str]] = {
    ...
    async def create(self, request: CheckpointCreate) -> CheckpointResponse:
        # A credit-schedule commit can preserve or delete an old cycle based on
        # checkpoint references.  Take the shared lock before target lookup so
        # a checkpoint cannot appear between that commit's recheck and delete.
    ...
    async def get_checkpoint(self, checkpoint_id: UUID) -> CheckpointResponse:
        checkpoint = await self.repository.get(checkpoint_id)
        if checkpoint is None:
            not_found(
                "reconciliation_checkpoint_not_found",
                "The reconciliation checkpoint does not exist",
            )
        return await self._response(checkpoint)
    ...
    async def attention(self) -> AttentionPage:
        ...
        return AttentionPage(items=[self._with_available_actions(item) for item in visible])

    async def open_checkpoint_difference_count(self) -> int:
        """Count latest open checkpoints without applying presentation dismissals."""
        count = 0
        for checkpoint in await self.repository.latest_by_target():
            if (await self._response(checkpoint)).state is ReconciliationState.OPEN:
                count += 1
        return count
    ...
    async def ignore_attention(
        ...
        if source_type in self._STATEMENT_IMPORT_ATTENTION_TYPES:
            invalid(
                "statement_import_attention_not_dismissible",
                "Statement import attention cannot be ignored",
            )
        if source_type not in self._DISMISSIBLE_ATTENTION_TYPES:
            invalid("attention_action_unknown", "This attention action is not available")
    ...
    def _with_available_actions(self, item: AttentionItem) -> AttentionItem:
        if item.source_type in self._DISMISSIBLE_ATTENTION_TYPES:
            action = AvailableAction(action="ignore", enabled=True)
        elif item.source_type in self._STATEMENT_IMPORT_ATTENTION_TYPES:
            action = AvailableAction(
                action="ignore",
                enabled=False,
                reason_code="statement_import_attention_not_dismissible",
                reason_message="Statement import attention cannot be ignored",
            )
        else:
            action = AvailableAction(
                action="ignore",
                enabled=False,
                reason_code="attention_action_unknown",
                reason_message="This attention type has no known safe action",
            )
        return item.model_copy(update={"available_actions": [action]})
