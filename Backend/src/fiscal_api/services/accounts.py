...
class AccountService:
    ...
    async def order_state(self) -> AccountOrderState:
        accounts = await self.repository.list(include_archived=False)
        impacts = await self.repository.balance_impacts([item.id for item in accounts])
        return AccountOrderState(
            items=[self.response(item, impacts.get(item.id, 0)) for item in accounts],
            list_revision=self._order_revision(accounts),
        )
    ...
    async def update(self, account_id: UUID, patch: AccountPatch) -> AccountResponse:
        ...
        check_version(
            ...
            reload_path=f"/api/v1/accounts/{account.id}",
        )
        ...
        if (
            ...
            conflict(
                "credit_schedule_preview_required",
                "A used credit schedule must be changed through preview and commit",
                details={
                    "reason": "credit_schedule_preview_required",
                    "preview_path": f"/api/v1/credit-accounts/{account.id}/schedule-change-preview",
                    "commit_path": f"/api/v1/credit-accounts/{account.id}/schedule-change",
                    "safe_to_reload": True,
                    "available_actions": ["preview_schedule_change"],
                },
            )
    ...
    async def reorder(
        self, ordered_ids: list[UUID], expected_list_revision: str | None
        ...
        current_list_revision = self._order_revision(accounts)
        if expected_list_revision is None:
            conflict(
                "list_revision_required",
                "Reordering requires the current account list revision",
                details={
                    "reason": "list_revision_required",
                    "order_scope": {"kind": "accounts"},
                    "reload_path": "/api/v1/accounts/order-state",
                    "safe_to_reload": True,
                },
            )
        if current_list_revision != expected_list_revision:
            conflict(
                ...
                "The account order changed by another request",
                details={
                    "reason": "list_changed",
    ...
    @staticmethod
    def _order_revision(accounts: list[Account]) -> str:
        return list_revision(
            "accounts",
            (
                (account.id, account.version, account.sort_order, account.archived_at)
