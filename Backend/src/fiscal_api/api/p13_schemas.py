...
class CashFlowEditable(APIModel):
    ...
    def validate_shape(self) -> "CashFlowEditable":
...
class CashFlowDraft(CashFlowEditable):
    recurrence: CashFlowRecurrence | None = None
    recurrence_end_date: date | None = None

    @model_validator(mode="after")
    def validate_recurrence(self) -> "CashFlowDraft":
...
class CashFlowReplace(CashFlowEditable):
...
class CashFlowSystemReplace(APIModel):
    title: str = Field(min_length=1, max_length=120)
    note: str | None = Field(default=None, max_length=500)
    # Deprecated compatibility input. Reimbursement amounts are derived from
    # the current reimbursement facts and this value is never persisted.
    planned_amount_minor: PositiveMinorUnits | None = Field(default=None, deprecated=True)
    expected_date: date
