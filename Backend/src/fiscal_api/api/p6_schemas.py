...
from fiscal_api.db.models import AccountKind, ReimbursementClaimStatus
...
class ReimbursementPartyDraft(APIModel):
    ...
    @classmethod
    def strict_expected_date(cls, value: date | str | None) -> date | str | None:
        """Accept only an ISO local business date, never a timestamp."""
        ...
        try:
            ...
            raise ValueError("expected_date must be an ISO date (YYYY-MM-DD)") from error
...
class ReimbursementClaimCommit(ReimbursementClaimReplace):
    preview_token: UUID
...
class ReimbursementCancelCommit(ReimbursementVersionRequest):
    preview_token: UUID
...
class ReimbursementReceiptCreateCommit(ReimbursementReceiptDraft):
    preview_token: UUID


class ReimbursementReceiptReplaceCommit(ReimbursementReceiptReplace):
    preview_token: UUID
...
class ReimbursementReceiptPreview(APIModel):
    ...
    claim_after: ReimbursementClaimResponse
    party_id: UUID
    amount_minor: int
...
class ReimbursementEligibilityReason(APIModel):
    code: str
    message: str
...
class ReimbursementExpenseCandidate(APIModel):
    transaction_id: UUID
    title: str
    business_date: date
    kind: str
...
class ReimbursementExpenseCandidatePage(APIModel):
    items: list[ReimbursementExpenseCandidate]
    next_cursor: str | None


class ReimbursementReceiptAccountOption(APIModel):
    id: UUID
    name: str
    kind: AccountKind


class ReimbursementReceiptAccountOptions(APIModel):
    items: list[ReimbursementReceiptAccountOption]
