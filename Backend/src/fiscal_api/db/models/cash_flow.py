...
class CashFlowSystemOverride(Base):
    ...
    note: Mapped[str | None] = mapped_column(Text, nullable=True)
    direction: Mapped[str] = mapped_column(String(16), nullable=False)
    # Reimbursement system overrides own presentation/status only. Their
    # amount is nullable because the live reimbursement fact is authoritative.
    planned_amount_minor: Mapped[int | None] = mapped_column(BigInteger, nullable=True)
    expected_date: Mapped[date] = mapped_column(Date, nullable=False)
    account_id: Mapped[UUID | None] = mapped_column(
        Uuid, ForeignKey("accounts.id", ondelete="RESTRICT"), nullable=True
    )
