...
class ReimbursementPreview(Base):
    """A short-lived, single-use server-side reimbursement preview session.

    Financial previews intentionally have their own persistence boundary: they
    are not revisions and must never be included in Archive payloads.
    """

    __tablename__ = "reimbursement_previews"
    __table_args__ = (
        CheckConstraint("char_length(request_hash) = 64", name="request_hash_length"),
        Index("ix_reimbursement_previews_expiry", "expires_at"),
    )
    ...
    receipt_id: Mapped[UUID | None] = mapped_column(
        Uuid, ForeignKey("reimbursement_receipts.id", ondelete="CASCADE")
    )
...
class ReimbursementOperation(Base):
    ...
    preview_id: Mapped[UUID | None] = mapped_column(
        Uuid, ForeignKey("reimbursement_previews.id", ondelete="RESTRICT"), unique=True
    )
    kind: Mapped[str] = mapped_column(String(32), nullable=False)
