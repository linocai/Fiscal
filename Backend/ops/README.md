...
```sh
docker compose -f Backend/ops/compose.local.yml up -d
docker compose -f Backend/ops/compose.local.yml ps
```
...
```sh
docker compose -f Backend/ops/compose.local.yml down
```
...
```sh
cp Backend/ops/staging.env.example Backend/ops/staging.env
chmod 600 Backend/ops/staging.env
```
...
```sh
docker compose --env-file Backend/ops/staging.env -f Backend/ops/compose.staging.yml config --quiet
docker compose --env-file Backend/ops/staging.env -f Backend/ops/compose.staging.yml build api
docker compose --env-file Backend/ops/staging.env -f Backend/ops/compose.staging.yml run --rm api alembic upgrade head
docker compose --env-file Backend/ops/staging.env -f Backend/ops/compose.staging.yml up -d
docker compose --env-file Backend/ops/staging.env -f Backend/ops/compose.staging.yml ps
```
...
1. Set `FISCAL_IMAGE_TAG` in `Backend/ops/staging.env` to a new rollback tag.
