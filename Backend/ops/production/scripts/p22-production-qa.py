...
def request(key: str, path: str, method: str = "GET", body: dict[str, object] | None = None):
    ...
    http_request = Request(  # noqa: S310 - BASE_URL is a fixed HTTPS origin
        BASE_URL + path,
        data=data,
        method=method,
        headers={"Authorization": f"Bearer {key}", "Content-Type": "application/json"},
    )
    return urlopen(http_request, timeout=20)  # noqa: S310 - fixed HTTPS request


def main(argv: list[str] | None = None) -> None:
    argparse.ArgumentParser(
        description="Run the single stdin-only P22 production QA receipt check"
    ).parse_args(argv)
    ...
    try:
        with request(
            ...
            "/categories",
            "POST",
            {"name": name, "direction": "expense", "icon": "tag", "color_hex": "#123456"},
        ) as response:
        ...
        with request(
            key, f"/categories/{created['id']}?expected_version={created['version']}", "DELETE"
        ) as response:
            revision = response.headers.get("X-Fiscal-Data-Revision")
            scopes = response.headers.get("X-Fiscal-Affected-Scopes")
        ...
        print(
            "production_qa=status=create201/delete204 revisions=1,2 scopes=full9 cleanup=complete"
        )
    finally:
        if created is not None:
            try:
                with request(
                    key,
                    f"/categories/{created['id']}?expected_version={created['version']}",
                    "DELETE",
                ):
                    pass
