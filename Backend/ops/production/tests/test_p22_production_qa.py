...
class ProductionQATests(unittest.TestCase):
    def test_success_and_parse_failure_still_delete(self) -> None:
        headers = {
            "X-Fiscal-Data-Revision": "1",
            "X-Fiscal-Affected-Scopes": ",".join(qa.EXPECTED_SCOPES),
        }
        deleted = {"count": 0}

        def open_(request, timeout=20):
            if request.method == "POST":
                return Response({"id": "id", "version": 1}, headers)
            deleted["count"] += 1
            return Response(
                {},
                {
                    "X-Fiscal-Data-Revision": "2",
                    "X-Fiscal-Affected-Scopes": headers["X-Fiscal-Affected-Scopes"],
                },
            )

        with (
            patch.object(qa, "urlopen", open_),
        ...
        bad = dict(headers)
        bad.pop("X-Fiscal-Affected-Scopes")
        deleted["count"] = 0

        def badopen(request, timeout=20):
            if request.method == "POST":
                return Response({"id": "id", "version": 1}, bad)
            deleted["count"] += 1
            return Response({}, {})

        with (
            self.assertRaises(RuntimeError),
            patch.object(qa, "urlopen", badopen),
            patch("sys.stdin", io.StringIO("secret\n")),
        ):
            qa.main([])
    ...
    def test_delete_failure_is_explicit_and_secret_is_not_output(self) -> None:
        ...
        output = io.StringIO()
        with (
            self.assertRaisesRegex(RuntimeError, "cleanup delete failed"),
            patch.object(qa, "urlopen", fail),
            patch("sys.stdin", io.StringIO("SENTINEL_SECRET\n")),
