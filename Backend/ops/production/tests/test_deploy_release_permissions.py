...
class DeployReleasePermissionsTests(unittest.TestCase):
    ...
    def test_deploy_rejects_legacy_lowercase_systemd_layout(self) -> None:
        ...
        self.assertIn("still references the legacy repository layout", DEPLOY)

    def test_release_access_dry_run_is_explicit_and_has_no_apply_side_effects(self) -> None:
        result = subprocess.run(  # noqa: S603 - fixed local maintenance script
            ["/bin/bash", str(BOOTSTRAP_PATH), "--release-access"],
            check=False,
            capture_output=True,
            text=True,
        )
    ...
    def test_restore_verify_uses_migrator_for_release_alembic_only(self) -> None:
        ...
        expected_head_block = RESTORE_VERIFY[
            expected_head : RESTORE_VERIFY.index('[[ -n "$actual_head"', expected_head)
        ]
    ...
    def test_p22_shadow_wrapper_fixes_workspace_and_cwd_contract(self) -> None:
        self.assertIn(
            'install -d -o root -g fiscal_migrator -m 0710 "$evidence_parent"', SHADOW_WRAPPER
        )
        self.assertIn(
            'install -d -o fiscal_migrator -g fiscal_migrator -m 0700 "$workspace"', SHADOW_WRAPPER
        )
        ...
        self.assertIn(
            "P22 source/principal preflight passed; no target database was used", SHADOW_WRAPPER
        )
    ...
    def test_p22_shadow_archive_wrapper_keeps_credentials_off_argv_and_cleans_them(self) -> None:
        ...
        self.assertIn('rm -f -- "$source_env" "$target_env" "$password_file"', SHADOW_WRAPPER)
        self.assertIn(
            'exec "$python_bin" -m "$archive_module" "$@" < "$archive_password"', SHADOW_WRAPPER
        )
    ...
    def test_p22_shadow_controlled_env_overrides_a_hostile_inherited_dsn(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            result = subprocess.run(  # noqa: S603 - fixed shell with controlled arguments
                [
                    "/bin/bash",
                    "-ceu",
    ...
    def test_p22_shadow_archive_modes_keep_source_and_target_contracts_separate(self) -> None:
        ...
        self.assertIn(
            "target must be an explicitly scoped fiscal_p22_shadow database", SHADOW_WRAPPER
        )
        self.assertIn(
        ...
        self.assertIn(
            'run_archive "$target_env" fiscal_api.cli.archive "$archive_path" '
    ...
    def test_p22_shadow_provisioning_rejects_existing_and_proves_migrator_control(self) -> None:
        ...
        self.assertIn(
            'createdb --template=template0 --owner=fiscal_migrator "$target_database"',
            SHADOW_WRAPPER,
        )
        ...
        self.assertIn(
            "CREATE TABLE $probe_table (id integer); DROP TABLE $probe_table", SHADOW_WRAPPER
        )
