...
- P28-A：`9b77e07` 已 Verified。mac consent intake 在 security-scoped temp workspace 完成 metadata/hash、duplicate 与 P24 `register → start → redacted evidence`；无 PDF/image/path/bookmark/raw filename 上传或持久化，响应丢失不自动重发。macOS **116/21**、mac/iOS Debug、P24 PG **4**、PG JUnit **266/0/0** 通过，见 [P28 QA](docs/qa/p28/results.md)。
- P28-B：`d941eeb` / `1b0d89a`、QA `be7f7b0` 已 Automated Verified。macOS 三栏审核仅在 `review_available=true` 可写；无 run 如实 evidence-only。Apple targeted **8 passed**、两端 Debug build；fresh PG targeted **13 passed**、全量 JUnit **268/0/0**，见 [P28 QA](docs/qa/p28/results.md)。无 PDF/image、Provider、confirm、iOS UI 或生产。
- **P28-C 已冻结，尚未实施：仅 macOS、仅 `review_available=true`。** 用户只能显式选未确认、非 `unresolved` 的非空子集；“预览确认”读最新状态，打开 sheet 不写账。`review_available=false`、空/重复/缺失/已确认/unresolved 行不能进入；P26、PDF/image/cache、iOS、自动重试和生产继续禁止。
- **需要 additive read DTO，而非客户端自行拼装。** 现有 workbench 只含 final-draft version，缺跨页 selected snapshot、final draft 金额、confirmed/frozen 状态和完整 check 摘要，不能诚实给出确认前事实。新增无副作用 `POST /statement-imports/{id}/confirmation-preview`，只收 `row_ids`（1–1000），返回 batch status/version、selected row 的 row/draft/final expected versions、resolution/frozen state、create/match/two-ignore/unresolved counts、已知金额分项/unavailable 数、passed/failed/unavailable checks；不得返回 PDF/image/evidence/provider body 或写 revision/ledger/receipt。另加带 `Idempotency-Key` header 的只读 receipt lookup，只有持久化 P27 receipt 才声明结果；workbench 仅加 `is_confirmed` 以展示冻结。
- sheet 必须显示 selected 与 batch unresolved count、各 resolution 数、金额摘要（未知明确标未知）和 failed/unavailable 警示；不默认聚焦 Confirm、不设键盘 shortcut、Enter/onSubmit 不确认。用户点 final action **之后**才生成 UUID key，并将 preview 原样 versions/rows 送既有 `POST .../confirm`；P27 仍逐项重验并一次事务全有或全无。409 丢弃 preview、Reload 后重新选择；传输/响应丢失进入 response-unknown，绝不重 POST，用户仅可显式 lookup receipt。receipt 显示 operation/status/confirmed row IDs；partial-confirmed 的已确认行永久 frozen，余行仅可按既有已保存 resolution 另批确认，P28-C 不解冻、改写或自动确认。
- 当前：P24–P28-B 自动门已验证；下一最小块为 P28-C explicit final-confirmation sheet。它只接 P27-B 原子 confirm，不能解除完整 P28/v1.4 发布门。
...
执行 **P28-C：macOS final-confirmation sheet（Apple 主责；backend 仅 preview/receipt 读取面）**。Backend 拥有 `confirmation-preview` schema/service/route、receipt lookup、`is_confirmed` workbench field 和 PG tests；preview/lookup 不得拿 mutation lock、创建 operation、改 batch/row/revision、调用 provider 或写 ledger。既有 P27 `/confirm`、idempotency、domain-service、Archive entity/migration 不改。Apple 新增唯一 `StatementImportConfirmationRepository`、preview/receipt/ephemeral operation models 与 `MacStatementImportConfirmationSheet`，接入既有 host；只在 final button 调 confirm，复用 P27 request/receipt DTO，不改 P28-A intake 或 iOS composition。

验收门：deterministic mock/URLProtocol 覆盖 review gate、empty/subset selection、preview exact versions/counts/checks/unknown amount、final action 前零 key/confirm、final action 后单一 UUID header+exact body、409 reload/re-preview、response-loss 零 POST 重发和显式 receipt lookup；partial/frozen/receipt 逐项可见。VoiceOver 宣读警示/不可逆性/unknown，keyboard 可到达但 Enter/Cmd 不可确认，1040×700/1280×820 snapshot/interaction 通过。backend PG 覆盖 preview/lookup 零写、P27 idempotency/rollback/partial freeze、Archive restore/replay/provenance/revision；P24/P27/P28 targeted 与 fresh PG full（当前 268）全绿。Apple targeted/macOS tests、macOS+iOS Debug builds 通过；红线断言无 Provider/PDF/image/cache/自动重试/iOS confirm UI。结果续写 [P28 QA](docs/qa/p28/results.md)，再评估完整 P28，绝不部署。
