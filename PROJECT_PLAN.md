...
- [2026-07-21] 快修（Build 20 + nginx）：AI 待确认「点确认卡一下/第二条报服务器未响应」= nginx `fiscal_ai` 严限流（10r/m）罩住全部 /ai/ 路径，确认连点即 429，客户端把无信封 429 误报成「响应无法解析」，且 isMutating 扣住 6 请求串行链数秒。三合一修复：①nginx zone 改 method+uri map 键只计 POST create/retry，更名 `fiscal_ai_parse` 零停机换 key（reload 不允许改既有 zone key，已实测验证边界）；②客户端新增 `rateLimited` 错误（诚实文案）、幂等 GET 退避重试一次、429 后保留幂等键；③execute 后立即释放按钮，账本/报表刷新合并为单一后台任务。88 测试全绿，Build 20 已换包。
- [2026-07-20] P19 热修二（Build 19）：iPhone 连不上的根因是 **Debug 安装指向 localhost**——Xcode 直接 Run 用 Debug 配置、`FISCAL_API_BASE_URL=127.0.0.1:8000`，且连接卡 failed 态不渲染口令框成死胡同。修复：FiscaliOS scheme Run 钉到 Release（真机安装永远指生产）、口令框在 failed/offline 态也渲染、连接卡显示当前服务器主机名（错装一眼可见）。85 测试全绿、iOS Release + mac Release 构建过。mac 保持 Build 18 未动。
- [2026-07-19] P19 热修（Build 18）：v1.2.4(17) 的 mac 端把 access_key 写成 iCloud 同步钥匙串条目——Developer ID 无 provisioned entitlement 必然失败；服务端 initialize 已成功（凭证已建、旧层已关）但返回的 key 被丢弃，mac 困在未授权循环、报错文案误导。修复：AccessKeyStore 平台感知（iOS 优先同步条目+本地降级，mac 纯本地，读/删兼容两形态）、失败文案改为「口令已验证、保存失败请重试」、双端钥匙串文案改平台准确。85 Swift 测试全绿（存储形态测试重写），Build 18 已换包。用户重输已设口令即恢复连接；iPhone 直接装 Build 18。
- [2026-07-19] 快修（用户拍板「一把梭」）：iOS 每次更新丢设备密钥。诊断：服务端旧令牌全部仍 active（`kkk` 等成为孤儿行），丢失发生在手机端安装环节；07-17/18 共 4 个一次性密钥 pending 过期，说明手动配对流程本身易失败。两层修复——①iOS Keychain 条目钉显式 team access group（`KeychainTokenStore` 增 accessGroup 参数，mac 传 nil）；②mac 签发界面出 `fiscal://pair?token` 二维码 + iOS 注册 fiscal URL scheme（xcodegen info.properties 声明，手改 Info.plist 会被重写）+ `onOpenURL` 自动激活，扫码即配对，零输入，安全模型不变。86 Swift 测试全绿，双端构建过，mac Build 16 已换包；iOS 待用户自装后验证扫码链路与下次更新密钥留存。遗留：服务端 `kkk` 等孤儿 active 令牌建议在 mac 设备列表里手动吊销。
- [2026-07-19] 生产部署（用户授权）：release `a68bc383d398`（= main HEAD，含解析对称校验与信用消费自动改判）经 deploy.sh 上线——服务器端 144 测试、迁移前验证备份、readiness、公网冒烟全过；`/opt/fiscal/source` 从已删除的 codex 分支切至跟踪 main。mac Build 15 此前已换包，两端与 main 一致；后端不再有待部署项。
- [2026-07-19] 快修（用户拍板）：解析层「支出+信用账户」自动改判为信用消费并保留账户（reason `credit_purchase_reclassified`，凭 kind 门槛与 reason code 双重排除在自动执行外，收入+信用账户仍抹账户不猜）；后端 144 测试 / ruff / pyright 全绿，**生产后端两笔待部署**（对称校验+自动改判）；mac Build 15 已换包（备份 build14）。
- [2026-07-19] 快修收口：根因实锤——AI 把花呗扣款解析成 expense+花呗，解析校验抹掉信用账户后提案带空账户仍可执行，422 不可见。已修：解析层对 credit_purchase/transfer/repayment 补对称账户与分类校验（新 reason `destination_kind_mismatch`，后端 141 测试全绿，**生产后端待部署**）；双端确认按钮按 executionBlockers 阻断并提示「先编辑补全」；mac 检查器补 reviewWarnings 与主数据加载。Build 14 已换包（备份 build13）。
- [2026-07-19] 快修：mac「AI 待确认」点确认无反应 = mac 检查器完全不渲染错误（服务端任何 4xx 拒绝都表现为死按钮）。本地一次性 Postgres 复现矩阵证实消费/执行后端路径健康（自然月/换模式后均 OK），可致 4xx 的是提案钉 cycle、kind-账户错配（解析层对 credit_purchase 缺账户类型校验）或版本冲突。已给 mac 检查器补 iOS 同款错误横幅、账户/分类行与按钮忙态；Build 13 已换包（备份 build12）。待用户重点确认后读横幅定最终根因；解析层校验缺口留待下一步。
- [2026-07-18] 独立审计收尾：Fable 5 亲自完成 v1.2.3 全量审计（报告 `archive/code-audit-2026-07-18.md`），修复 3 个确定 bug——iOS 云连接刷新改为分别刷新 overview/spending 两个报表实例（原 `loadAll()` 漏刷总览且竞写 phase）、删除 `accountRows` 死代码及 `drillDownRows` 不可达 cashFlow 分支、移除失效的 `IOSReportsScreen.initialLens`；83 项 Swift 测试与双端 Debug 构建通过。按用户「不用分支开发」决定，`codex/v1.2.1-credit-cashflow` 已快进合入 main 并删除（本地+远端），后续一律在 main 施工。遗留：v1.2.2/v1.2.3 tag 未打、P18 真机验收未闭环、审计 B 节 3 项结构性风险待排期。
