...
- **P29-A 已冻结，尚未实施：仅 iOS/Kurisu 的本地 intake、分步审核和显式确认，完全复用 P24–P28 API。** `More → 账单导入` 以 Files `fileImporter(.pdf)` 选本地文件；security scope 仅覆盖复制到随机 temp workspace 的时段，PDF/CGImage/URL/bookmark/原名/原文均不持久化、不进 DTO/日志或网络。先显示本机名称/页数/大小/SHA、duplicate 与同一 consent，再严格 `register → start → deterministic masked JSON evidence`；未 consent 零 API，response-loss 零自动 resend，Retry 只发内存 package，App/background 后无 package 则只可显式 GET batch 或重新选择。
- 现有 PDFKit+Vision extractor 已在 `FiscalKit_iOS` 编译，限制仍为 **20 MiB/50 页/12M pixel/100k OCR chars**；iOS 只在前台 `Task` 处理，Files provider 可在 consent 后撤销 access 时如实失败并要求重选，不建 background task/queue、offline PDF/image cache 或永久 bookmark。小屏不用三栏：先显示 failed/unavailable checks、unresolved/疑似匹配，再可访问完整分页行表与已存 masked evidence；`review_available=false` 只能 evidence-only/source-unavailable，绝不伪造 PDF 或编辑/confirm。
- `review_available=true` 才允许 P27 五种 resolution/final-create-draft；账户/分类/信用字段仍全显式、archived 不可选、无候选默认值。每次编辑/prepare 以无缓存、含所选 cursor 的 fresh batch/row/draft/final version 重读；409 清空选择/本地表单并 Reload，跨设备无合并。P28-C preview/sheet/receipt 原样复用：无快捷键或 Enter confirm，最终 tap 后才 UUID+一次 POST；响应未知只允许显式 receipt lookup，partial/frozen 行不可编辑或再选。现有工程无 LocalAuthentication/Face ID seam，P29-A 不添加或声称设备认证。
- Attention 只加可由现有批次状态导出的只读 `statement_import_review`（`review_required|ready_to_confirm|partially_confirmed`）与 `statement_import_failed` 项；不含文件名、金额、证据或 Provider 内容。iOS 从既有 attention items 计待处理批次数并路由 batch ID，statement item 无“忽略”操作；不建新提醒表、推送或后台轮询。P29-A QA 仅可声明 Automated；Kurisu 签名 Debug 真机合成 fixture 是独立 Physical Device 门，真实账单、Provider、production migration/deploy、release manifest、tag/push 仍需用户当时单独授权。
- 当前：P24–P28-C 自动门已验证；下一最小块为 P29-A iOS local/review/confirm workbench 与 Attention 只读入口，不解除 v1.4 发布门。
...
执行 **P29-A：iOS/Kurisu statement-import flow（Apple 主责；backend 仅 Attention read projection）**。Backend 只在既有 `GET reconciliation/attention` 派生两类 import item、PG read-contract tests；无 migration/new entity/revision、provider、P24–P28 route/Archive/confirm 语义变更。Apple 将已有 intake/review/P27/P28C DTO+repositories 编入 `FiscalKit_iOS`，在 `FiscaliOSApp` 注入同一 transport/models；新增 iOS-only Files intake、step review/evidence/detail/confirmation screens 和 `IOSRootView`/Attention batch route。分页 helper 必须能精确取所选行，不能沿用首 200 行猜 version；source access、temp cleanup、foreground cancel 和内存清理均归 iOS flow。

验收门：synthetic local processor/URLProtocol+FiscaliOS UI tests 覆盖 Files consent/duplicate/temp cleanup/scene interruption、masked-only redlines、evidence-only、exception-first/full pagination、五种 resolution/archived refs、two-device stale 409 reload、preview exact body+final single UUID POST、response-loss zero resend/explicit receipt、partial freeze 和 Attention routing/count/privacy。iOS Simulator test/build、macOS regression build、fresh PG Attention+P24/P27/P28 targeted/full（当前 269）均绿；Kurisu 仅签名 Debug 的合成 PDF 手工流程和 VoiceOver/Dynamic Type/rotation/memory check 另记 Physical Device。结果写 [P29 QA](docs/qa/p29/results.md)；生产门仍是备份/影子 migration/Archive/守恒、两端真实受控账单、production smoke 与用户授权，P29-A 不执行。
