...
> 控制面版本：v1.3.0 施工版 ｜ 更新：2026-08-11 ｜ 状态：P20 自动、生产与 macOS/Kurisu 验收已完成；仅外部异地备份/真实告警服务未配置，不得进入 P21
...
**当前状态**：`81d5881` 已部署至生产、Alembic `20260811_0020`，自动与隔离恢复门禁已通过，旧 device-token 兼容层及表已移除。macOS 与 Kurisu 的新包均已连通生产。异地副本与真实告警接收器尚未配置，是唯一外部服务门；以上未闭环前不进入 P21。
