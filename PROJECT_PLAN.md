...
> 控制面版本：v1.3.0 施工版 ｜ 更新：2026-08-11 ｜ 状态：P20 已完成自动/生产收口，等待 Kurisu 解锁验收与外部备份/告警服务；不得进入 P21
...
**当前状态**：`81d5881` 已部署至生产、Alembic `20260811_0020`，自动与隔离恢复门禁已通过，旧 device-token 兼容层及表已移除。macOS 的新包已连通；Kurisu 的新包已安装但受锁屏阻断启动。异地副本与真实告警接收器尚未配置，是外部服务门。以上未闭环前不进入 P21。
