...
> 目标版本：v1.5.2（26）｜更新：2026-08-23（Asia/Shanghai）｜阶段：**RELEASE · 本地发布完成**
...
- 用户已授权完成修复、独立复审、commit、签名、macOS 换包安装、tag 与 push；iOS 安装由用户执行。
...
- v1.5.2（26）源码、审计证据、签名 Release 产物和校验清单均已收口；macOS 签名包已严格验签、换包安装并成功启动。
- iOS 真机开发签名 IPA 已产出，安装由用户执行；本轮未执行 Apple 公证、TestFlight、Backend 变更或生产部署。
- 发布产物、SHA-256、回滚备份和最终 Git 状态见 `archive/releases/v1.5.2/RELEASE_STATE.md`。
