...
- **P28-B 已冻结，尚未实施：从 P28-A 的 `reviewRequired(batchID)` 进入同一 macOS 工作台；它只读服务端脱敏 evidence，并不承诺已有 P27 结构化审核。** P28-B backend 仅增无 migration/写账的 `GET /statement-imports/{id}/review-workbench?cursor&limit&filters` 与 `GET .../review-workbench/pages/{page}`：前者给 batch/version、checks、行版本/draft/candidate/final-draft-version，`review_available=false` 时只给 evidence-only；后者只给已存 `source_kind`、masked page/row text 与 normalized boxes。绝不返回 PDF/image/URL/path/raw Provider body；缺页/空证据显示“原件未保留，脱敏来源不可用”，绝不重读本地 PDF。
- P28-A 新完成的 batch 尚无 P26 `validated_result`，故默认是 evidence-only/结构化审核不可用；P28-B 不 POST provider attempt 或 validation run（synthetic/real 均禁止）。仅 `review_available=true` 的既有 P27 run 可读 checks/candidates/drafts 并写**既有** versioned `draft-resolution`/`final-create-draft` APIs；resolution 需要 batch/row/draft versions，final draft 需要其 version。所有 PUT 或响应丢失都先 reload、无本地合并/自动重发；409 显示“服务器已变化”，用户重新选择后才提交。
- 右侧 inspector 只显示候选与当前草稿，`match_existing` 必从当前候选显式选取，intentional ignore 必填原因；`create_new` 先显式选 kind，再在未持久化的本地表单选择日期/金额/title/note、账户/目标账户、分类和所需信用字段。复用 Accounts/Categories（及现有 credit read model）的稳定 ID 并过滤 archived；候选绝不自动填 kind/account/category/amount/date，当前 API 不可表达的语义保持 unresolved。review response、masked evidence 和本地未保存表单均是内存态，repository 一律 `cache:false`、视图离开清空，无 PDF/image/offline cache/background retry。
- 布局固定为左侧批次/check/filter 摘要、中间已存脱敏证据或 source-unavailable、右侧虚拟化行表与 inspector；筛选只按服务端 current run 的 resolution/candidate/check/evidence state。P28-B host 不改 P28-A intake 逻辑，只观察其公开 `reviewRequired` 状态路由，Back 返回。VoiceOver 明示三栏/脱敏/不可用，键盘可到达 filter、行、编辑/Reload；在现有 1040×700 最小、1280×820 默认窗口下主线程仅更新 selection/progress。
- 当前：P27 核心确认与 P28-A intake 已验证；下一最小块为 P28-B read/edit workbench。它不含 confirm/action/sheet、批量确认、iOS UI、P26、后台上传或生产，不能解除完整 P28/v1.4 发布门。
...
执行 **P28-B：macOS statement review workbench（Apple 主责；backend 仅 additive redacted read DTO）**。Backend 只拥有上述两个 read-only endpoint、cursor/filter 验证和 PG API tests；不得改 P24–P27 写入语义、provider/confirmation route、migration 或 Archive model。Apple 新增 `StatementImportReviewWorkbenchRepository`（唯一 P28-B remote adapter）、workbench/list/evidence/inspector models 与 `MacStatementImportWorkspaceHost`/三栏 view；host 以 P28-A public state 路由，不改 intake 文件。现有 resolution/final-draft repositories 是仅有写入面；复用 account/category/credit read models，不新增自动分类或默认值。

验收门：deterministic mock/URLProtocol 覆盖 P28-A→workbench 路由、evidence-only（零 provider POST）、masked-only payload/`cache:false` 红线、source unavailable、cursor/filter/selection、五种 resolution、显式 final draft/archived master exclusion、409/reload/response-loss zero resend，以及离开清理。断言没有 PDF/image/path/bookmark/raw filename/provider/confirm endpoint/Idempotency-Key；无 iOS workbench UI。backend read DTO PG contract、P24/P27 targeted 与 fresh PG full（当前 266）全通过；Apple targeted/macOS tests、macOS+iOS Debug builds、VoiceOver/keyboard 与 1040×700/1280×820 snapshot/interaction checks 通过。结果续写 [P28 QA](docs/qa/p28/results.md)，再评估完整 P28，而非进入确认或生产。
