...
> 计划版本：v0.4（施工权威版）
> 日期：2026-07-17
> 状态：P17 v1.2.1 信用账期、现金流与交易分期统一已发布
...
- 当前施工位置：P17 v1.2.1（Build 11）。目标是以信用流水/账期为唯一金额真相，信用还款现金流改为不可编辑的动态投影；「记一笔·信用消费」内原子创建交易分期；账户新增普通账单日截止/上个自然月两种账期模式。旧 `credit_cycle` 现金流覆盖将清除并重新按真实剩余负债待核对；已结清历史不重写，未结清账期通过预览+原子应用重算。
