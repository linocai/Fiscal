...
def upgrade() -> None:
    op.create_table(
        ...
        sa.CheckConstraint("(target_kind = 'account' AND account_id IS NOT NULL AND credit_cycle_id IS NULL) OR (target_kind = 'credit_cycle' AND account_id IS NULL AND credit_cycle_id IS NOT NULL)", name="target_shape"),
        sa.ForeignKeyConstraint(["account_id"], ["accounts.id"], ondelete="RESTRICT"),
        sa.ForeignKeyConstraint(["credit_cycle_id"], ["credit_cycles.id"], ondelete="RESTRICT"),
        sa.PrimaryKeyConstraint("id"),
    )
    op.create_index("ix_reconciliation_checkpoints_target_time", "reconciliation_checkpoints", ["account_id", "credit_cycle_id", "as_of"])
    op.create_table(
...
def downgrade() -> None:
    ...
    op.drop_index("ix_reconciliation_checkpoints_target_time", table_name="reconciliation_checkpoints")
